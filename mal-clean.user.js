...
function createDisplayBox(cssProperties, windowTitle) {
  let displayBox = AdvancedCreate(
    "div",
    "maljsDisplayBox",
    false,
    document.querySelector("#myanimelist")
  );
  if (windowTitle) {
  ...
  let displayBoxClose = AdvancedCreate(
    "span",
  ...
  let resizePearl = AdvancedCreate(
    "span",
    "maljsResizePearl",
    false,
    displayBox
...
function nativeTimeElement(e) {
  ...
  return (function e() {
    let r =
      Math.round(new Date().valueOf() / 1e3) - Math.round($.valueOf() / 1e3);
    if (0 === r) return "Now";
...
function aniMangaAddClass(main, name) {
  ...
  if (h2.length > 0) {
    ...
    parent.is("div") &&
    !parent.hasClass("leftside") &&
    !parent.hasClass("rightside")
...
function peopleDetailsAddDiv(title) {
  ...
  let divNameText =
    divNameElement[0] && divNameElement[0].nextSibling
      ? divNameElement[0].nextSibling.nodeValue.trim()
      : null;
  let newDiv = $('<div class="spaceit_pad"></div>').html(
    title + " " + divNameText
  );
...
function handleEmptyInfo(divSelector, checkText, mode) {
  ...
  if ($div.length) {
    ...
    if (
      nextSibling &&
      !$(nextSibling).attr("itemprop") &&
      (nextSibling.nodeValue || nextSibling.innerText) &&
      (nextSibling.nodeValue || nextSibling.innerText).includes(checkText)
    ) {
  ...
  try {
    const response = await fetch(
      `https://myanimelist.net/${
        type ? "manga" : "anime"
      }.php?o=9&c%5B0%5D=a&c%5B1%5D=d&cv=2&w=1&show=${page ? page : "0"}`
    );
    ...
    rows.forEach((row) => {
      const imgSrc = row.querySelector("td img")
        ? row
        ...
        : "";
      if (imgSrc) {
        const title = row.querySelector("td strong")
          ? row.querySelector("td strong").textContent
          : "";
        const type = row.querySelector('td[width="45"]')
          ? row.querySelector('td[width="45"]').textContent.trim()
          : "";
        const url = row.querySelector("td a")
...
function updateRecentlyAddedSliders(slider, leftSlider, rightSlider) {
  ...
  $(".widget-container.left.recently-anime i")
    .on("click", async function () {
      ...
      createInfo(
        clickedFrom,
        ".widget-container.left.recently-anime .anime_suggestions"
      );
    })
  ...
  $(".widget-container.left.recently-manga i")
    .on("click", async function () {
      ...
      createInfo(
        clickedFrom,
        ".widget-container.left.recently-manga .anime_suggestions",
        1
      );
    })
    .on("mouseleave", async function () {
  ...
  if (!waitForInfo && $(".tooltipBody").length === 0) {
    ...
    if (!clickedSource.closest(".btn-anime")[0].getAttribute("details")) {
      ...
      if (info.title) {
        info =
          ...
          (info.title
            ...
            : "") +
          (info.synopsis
            ? '<br><div class="text"><b>Synopsis</b><br>' +
              info.synopsis.replace(/(\[Written by MAL Rewrite\]+)/gm, "") +
              "</div>"
            : "") +
          (info.genres && info.genres[0]
            ? '<br><div class="text"><b>Genres</b><br>' +
              info.genres
                .map(
                  (node) => "<a href='" + node.url + "'>" + node.name + "</a>"
                )
                .toString()
                .split(",")
                .join(", ") +
          ...
          (info.demographics && info.demographics[0]
            ...
            : "") +
          (info.type
            ? '<br><div class="text"><b>Type</b><br>' + info.type + "</div>"
            : "") +
          (info.rating
            ? '<br><div class="text"><b>Rating</b><br>' + info.rating + "</div>"
            : "") +
          (info.aired && info.aired.string
            ? '<br><div class="text"><b>Start Date</b><br>' +
              info.aired.string.split(" to ?").join("") +
              "</div>"
            : "") +
          (info.broadcast
            ? '<br><div class="text"><b>Broadcast</b><br>' +
              info.broadcast.string +
              "</div>"
            : "") +
          (info.episodes
            ? '<br><div class="text"><b>Episodes</b><br>' +
              info.episodes +
              "</div>"
            : "") +
          (info.trailer && info.trailer.embed_url
            ? '<br><div class="text"><b>Trailer</b><br>' +
              ...
              "<br>" +
              '<iframe width="700" height="400" class="movie youtube" frameborder="0" autoplay="0" allowfullscreen src="' +
      ...
      if (!isFailed) {
        ...
        $('<div class="tooltipDetails"></div>')
          .html(info)
          .appendTo(clickedSource.closest(".btn-anime"));
      }
    }
    ...
    $(
      '<div class="tooltipBody">' +
        ($(".tooltipBody").length === 0 &&
        clickedSource.closest(".btn-anime")[0].children[2]
...
async function infoExit(mainDiv) {
  ...
  async function handleTooltipHide() {
    ...
    if (
      $(".tooltipBody:hover").length === 0 &&
      $(mainDiv + " i:hover").length === 0
    ) {
      await delay(150);
      if (
        $(".tooltipBody:hover").length === 0 &&
...
async function getFriends(username) {
  ...
  try {
    while (true) {
      const response = await fetch(
        `https://api.jikan.moe/v4/users/${username}/friends?page=${page}`
      );
...
async function editAboutPopup(data, type) {
  return new Promise((resolve, reject) => {
    ...
    const popupClose = create("a", {
      id: "currently-closePopup",
      class: "fa-solid fa-xmark",
      href: "javascript:void(0);",
    });
    const popupBack = create("a", {
      class: "popupBack fa-solid fa-arrow-left",
      href: "javascript:void(0);",
    });
    ...
    const popupDataTextButton = create(
      "button",
      { class: "dataTextButton" },
      "Copy"
    );
    const popupLoading = create(
      ...
      "Loading" +
        '<i class="fa fa-circle-o-notch fa-spin" style="top:2px; position:relative;margin-left:5px;font-family:FontAwesome;word-break: break-word;"></i>'
    );
    const iframe = create("iframe", {
      src: "https://myanimelist.net/editprofile.php",
    });
    ...
    $(iframe).on("load", async function () {
      ...
      let isClassic = $iframeContents
        .find("#about_me_setting_2")
        .is(":checked");
      ...
      let userBlogPage =
        "https://myanimelist.net/blog/" + $(".header-profile-link").text();

      popupLoading.innerHTML =
        "Updating" +
        '<i class="fa fa-circle-o-notch fa-spin" style="top:2px; position:relative;margin-left:5px;font-family:FontAwesome"></i>';
      ...
      if ($(iframe).contents()[0].URL.indexOf("editprofile.php") === -1) {
        if (
          settingsFounded &&
          $(iframe).contents()[0].URL.indexOf("myblog.php") === -1
        ) {
          let $blogFound = null;
          ...
          if ($maljsBlogDivs.length) {
            $maljsBlogDivs.each(function () {
              ...
              if ($this.html().indexOf("malcleansettings") > -1) {
                $blogFound = $(this);
                let $editLink = $blogFound
                  .next()
                  .find('a[href*="/myblog.php?go=edit"]');
                if ($editLink.length) {
                  $editLink[0].click();
                }
              }
    ...
    function notFound() {
      iframe.remove();
      popupLoading.innerHTML =
        "You are not using classic about.<br><br>Please paste this code into a blog post on the first page so that the code will run automatically.<br><br>";
...
async function editPopup(id, type, add, addCount) {
  return new Promise((resolve, reject) => {
    ...
    const popupId =
      "/ownlist/" +
      (type ? type.toLocaleLowerCase() : "anime") +
      "/" +
      id +
      "/edit?hideLayout=1";
    const popupBack = create("a", {
    ...
    $(iframe).on("load", function () {
      ...
      if (
        $(iframe).contents().find("#hide-advanced-button")[0].style.display ===
        ""
      ) {
        $(iframe).contents().find("#hide-advanced-button")[0].click();
      }

      let decreaseEp = $(iframe)
        .contents()
      ...
      function checkEp() {
        let ep = parseInt(
          $(iframe)
            .contents()
            .find(
              "#add_anime_num_watched_episodes,#add_manga_num_read_chapters"
            )
            .val()
        );
        let lastEp = parseInt(
          $(iframe).contents().find("#totalEpisodes,#totalChap").text()
        );
        let day = $(iframe)
          .contents()
          .find("#add_anime_finish_date_day,#add_manga_finish_date_day")[0];
        let month = $(iframe)
          .contents()
          .find("#add_anime_finish_date_month,#add_manga_finish_date_month")[0];
        let year = $(iframe)
          .contents()
          .find("#add_anime_finish_date_year,#add_manga_finish_date_year")[0];
        let startDate = $(iframe)
          .contents()
          .find("#add_anime_start_date_month,#add_manga_start_date_month")
          .val();
        let endDate = $(iframe)
          .contents()
          .find("#add_anime_finish_date_month,#add_manga_finish_date_month")
          .val();
        if (svar.autoAddDate) {
      ...
      if (add) {
        ...
        let mangaVol = $(iframe)
          .contents()
          .find("#add_manga_num_read_volumes")[0];
        let mangaVolLast = parseInt(
          $(iframe).contents().find("#totalVol").text()
        );
        let status = $(iframe)
          .contents()
          .find("#add_anime_status,#add_manga_status")[0];
  ...
  return new Promise((resolve, reject) => {
    ...
    $(iframe).on("load", function () {
      ...
      if ($(iframe).contents().find("form > input.inputtext")[0]) {
        ...
        $(iframe)
          .contents()
          .find("#contentWrapper")[0]
          .setAttribute(
            "style",
            "top: 5px!important;min-height: auto;padding: 0;width:auto;margin-left:0!important"
          );
        $(iframe)
          .contents()
          .find("#myanimelist")[0]
          .setAttribute("style", "width: auto;padding: 0px 5px;");
        $(iframe).contents().find("form:has(input.inputtext)")[0].style.width =
          "auto";
        $(iframe).contents().find("#content > div > div")[0].style.width =
          "auto";
      }
...
async function createCustomDiv(appLoc, header, content, editData) {
  ...
  if (!document.querySelector("#customAddContainer")) {
    ...
    let appendLoc = appLoc
      ? appLoc
      : document.querySelector("#user-button-div");
    const isRight =
      appLoc === "right" || (appLoc && appLoc.classList[1]) ? 1 : 0;
    if (isRight) {
    ...
    const closeButton = create(
      "button",
      {
        class: "mainbtns btn-active-def",
        id: "closeButton",
        style: { width: "45px", float: "right", marginTop: "-5px" },
      },
      "Close"
    );
    ...
    $(newDiv).append(
      "<h4>" + (appLoc ? "Edit" : "Add Custom") + " Profile Element" + "</h4>"
    );
    ...
    previewButton.addEventListener("click", function () {
      ...
      ${
        isRight && svar.alstyle
          ...
          : `<h5 style="${
              svar.alstyle ? "font-size: 11px; margin: 0 0 8px 2px;" : ""
            }">${headerText}</h5>`
      }
    ...
    addButton.addEventListener("click", function () {
      ...
      if (headerText.length > 1 && contentText.length > 1) {
        ...
        const customElBase64 = LZString.compressToBase64(
          JSON.stringify(customElArray)
        );
        const customElbase64url = customElBase64.replace(/\//g, "_");
        if (editData) {
          editAboutPopup(
            [editData, `customProfileEl/${customElbase64url}`],
            "editCustomEl"
          );
        } else {
          editAboutPopup(
            `customProfileEl/${customElbase64url}`,
            "customProfileEl"
          );
        }
      }
    ...
    if (typeof sceditor !== "undefined") {
      ...
      sceditor.formats.bbcode.set("spoiler", {
        ...
        html: function (token, attrs, content) {
          ...
          if (attrs.defaultattr) {
            data +=
              ' data-desc="' + sceditor.escapeEntities(attrs.defaultattr) + '"';
          }
          return (
            "<blockquote" +
            data +
      ...
      sceditor.command.set("spoiler", {
        exec: function (caller) {
          var html =
            '<blockquote class="spoiler"><button>spoiler</button><span class="spoiler_content" style="display:none"></span></blockquote>';
      ...
      sceditor.command.set("div", {
        txtExec: function (caller, content) {
          var editor = this;
          var sce_div =
            '<div id="sce_divoptionsbox"><div class="sce_div-option" data-action="insertDiv">';
          sce_div +=
            '<label for="div-id">ID (Optional):</label><input id="div-id" type="text" placeholder=".id" />';
          sce_div +=
            '<label for="div-class">Class (Optional):</label><input id="div-class" type="text" placeholder="#class" />';
          sce_div +=
            '<label for="div-style">Style (Optional):</label><input id="div-style" type="text" placeholder="style" /><br>';
          sce_div +=
            '<input id="insert-div-btn" type="button" class="button" value="Insert"></input>';
          sce_div += "</div></div>";
          var drop_content = $(sce_div);

          // Handle div insertion
          drop_content.find("#insert-div-btn").click(function (e) {
            var divId = $("#div-id").val()
              ? ' id="' + $("#div-id").val() + '"'
              : ' id="mc-div"';
            var divClass = $("#div-class").val()
              ? ' class="' + $("#div-class").val() + '"'
              : "";
            var divStyle = $("#div-style").val()
              ? ' style="' + $("#div-style").val() + '"'
              : "";
            var divTag =
              "[div" + divId + divClass + divStyle + "]" + content + "[/div]";
      ...
      sceditor.formats.bbcode.set("iframe", {
        ...
        format: function (element, content) {
          ...
          var title = element.getAttribute("title")
            ...
            : "";
          var sandbox = element.getAttribute("sandbox")
          ...
          var loading = element.getAttribute("loading")
            ...
            : "";
          var referrerpolicy = element.getAttribute("referrerpolicy")
            ? " referrerpolicy=" + element.getAttribute("referrerpolicy")
            : "";
          var mergedAttributes =
            title + sandbox + allow + loading + referrerpolicy;
        ...
        html: function (token, attrs, content) {
          ...
          var referrerpolicy = attrs.referrerpolicy
      ...
      sceditor.command.set("iframe", {
        txtExec: function (caller) {
          var editor = this;
          var sce_iframe =
            '<div id="sce_iframeoptionsbox"><div class="sce_iframe-option" data-action="insertIframe">';
          sce_iframe +=
            '<label for="iframe-src">Iframe URL:</label><input id="iframe-src" type="text" placeholder="https://" />';
          sce_iframe +=
            '<label for="video-width">Width (Optional):</label><input id="iframe-width" type="text" placeholder="" />';
          sce_iframe +=
            '<label for="video-height">Height (Optional):</label><input id="iframe-height" type="text" placeholder="" />';
          sce_iframe +=
            '<label for="iframe-html-src">Iframe as HTML (Optional):</label><textarea id="iframe-html-src" placeholder="<iframe src=&quot;&quot;></iframe>" /></textarea>';
          sce_iframe +=
            '<input id="insert-iframe-btn" type="button" class="button" value="Insert"></input>';
          sce_iframe += "</div></div>";
          ...
          drop_content.find("#insert-iframe-btn").click(function (e) {
            ...
            var iframeWidth = $("#iframe-width").val()
              ? " width=" + $("#iframe-width").val()
              : "";
            var iframeHeight = $("#iframe-height").val()
      ...
      sceditor.formats.bbcode.set("video", {
        ...
        format: function (element, content) {
          ...
          var height = element.getAttribute("height") || 315;
          var autoplay =
            element.getAttribute("autoplay") === "" ? " autoplay=1" : "";
          var controls =
            element.getAttribute("controls") === "" ? "" : " controls=0";
      ...
      sceditor.command.set("video", {
        txtExec: function (caller) {
          var editor = this;
          var sce_video =
            '<div id="sce_videooptionsbox"><div class="sce_video-option" data-action="insertVideo">';
          sce_video +=
            '<label for="video-url">Video URL:</label><input id="video-url" type="text" placeholder="https://" />';
          sce_video +=
            '<label for="video-width">Width (Optional):</label><input id="video-width" type="text" placeholder="" />';
          sce_video +=
            '<label for="video-height">Height (Optional):</label><input id="video-height" type="text" placeholder="" />';
          sce_video +=
            '<div><label><input type="checkbox" id="video-autoplay" /> Autoplay</label></div>';
          sce_video +=
            '<div><label><input type="checkbox" id="video-muted" /> Muted</label></div>';
          sce_video +=
            '<div><label><input type="checkbox" id="video-loop" /> Loop</label></div>';
          sce_video +=
            '<div><label><input type="checkbox" id="video-controls" checked /> Controls</label></div>';
          sce_video +=
            '<input id="insert-video-btn" type="button" class="button" value="Insert"></input>';
          sce_video += "</div></div>";
          ...
          drop_content.find("#insert-video-btn").click(function (e) {
            ...
            var videoWidth = $("#video-width").val()
      ...
      sceditor.create(contentInput, {
        ...
        autoUpdate: true,
        toolbar:
          "bold,italic,underline,strike|size,center,right,colorpick|bulletlist,orderedlist|code,quote,spoiler|image,link,youtube|video,iframe,div",
        allowIFrame: true,
        allowedIframeUrls: [],
        toolbarExclude: null,
        parserOptions: {},
  ...
  $("html, body").animate(
    {
      scrollTop:
        $("#customAddContainer").offset().top - $(window).height() * 0.1,
    },
    500
  );
}
...
var reloadButton = create(
  "button",
  { class: "mainbtns", id: "reloadbtn" },
  "Refresh"
);
...
const buttonsConfig = [
  ...
  { id: "animeInfoDesignBtn", setting: "animeInfoDesign" },
...
let fgColorSelector = create("input", {
  class: "badgeInput",
  id: "fgcolorselector",
  type: "color",
});
let updateFgButton = create(
  "button",
  { class: "mainbtns", id: "privateProfile" },
  "Update"
);
let removeFgButton = create("button", {
  class: "mainbtns fa fa-trash",
  id: "customFgRemove",
});
...
//Private Profile
var privateButton = create(
  "button",
  { class: "mainbtns", id: "privateProfile", style: { width: "48%" } },
  "Private"
);
var removePrivateButton = create(
  "button",
  { class: "mainbtns", id: "privateRemove", style: { width: "48%" } },
  "Public"
);
...
privateButton.onclick = () => {
  if (username !== $(".header-profile-link").text()) {
    window.location.href =
      "https://myanimelist.net/profile/" + $(".header-profile-link").text();
  } else {
...
var hideProfileElButton = create(
  "button",
  {
    class: "mainbtns",
    id: "hideProfileElementsButton",
    style: { width: "45%" },
  },
  "Hide"
);
var hideProfileElUpdateButton = create(
  "button",
  {
    class: "mainbtns",
    id: "hideProfileElementsUpdateButton",
    style: { width: "45%" },
  },
  "Update"
);
var removehideProfileElButton = create("button", {
  class: "mainbtns fa fa-trash",
  id: "hideProfileElementsRemove",
});
...
hideProfileElButton.onclick = () => {
  if (username !== $(".header-profile-link").text()) {
    ...
    if (hideProfileElButton.textContent === "Hide") {
      divIds.forEach((divId) => {
        ...
        if (div && !$(div).next().is(".hide-button")) {
          ...
          hideButton.textContent = hiddenProfileElementsTemp.includes(divId)
            ? "Show"
            : "Hide";
          ...
          hideButton.addEventListener("click", () => {
            hideButton.textContent =
              hideButton.textContent === "Hide" ? "Show" : "Hide";
            if (hiddenProfileElementsTemp.includes(divId)) {
              hiddenProfileElementsTemp = hiddenProfileElementsTemp.filter(
                (className) => className !== divId
              );
              div.style.opacity = "1";
              div.style.pointerEvents = "auto";
            } else {
    ...
    hideProfileElButton.textContent =
      hideProfileElButton.textContent === "Hide" ? "Cancel" : "Hide";
  }
};

hideProfileElUpdateButton.onclick = () => {
  const pfElBase64 = LZString.compressToBase64(
    JSON.stringify(hiddenProfileElementsTemp)
  );
...
var customProfileElUpdateButton = create(
  "button",
  {
    class: "mainbtns",
    id: "hideProfileElementsUpdateButton",
...
var customProfileElRightUpdateButton = create(
  "button",
  {
    class: "mainbtns",
    id: "hideProfileElementsUpdateButton",
...
var bgButton = create(
  "button",
  { class: "mainbtns", id: "custombg" },
  "Update"
);
var bgRemoveButton = create("button", {
  class: "mainbtns fa fa-trash",
  id: "custombgRemove",
});
...
var pfButton = create(
  "button",
  { class: "mainbtns", id: "custompf" },
  "Update"
);
var pfRemoveButton = create("button", {
  class: "mainbtns fa fa-trash",
  id: "custompfRemove",
});
...
//Custom CSS
var cssButton = create(
  "button",
  { class: "mainbtns", id: "customcss" },
  "Update"
);
var cssRemoveButton = create("button", {
  class: "mainbtns fa fa-trash",
  id: "customcssRemove",
});
var cssInfo = create("p", { class: "textpb" }, "");
let cssInput = create("input", { class: "cssInput", id: "cssInput" });
var cssAlStyle = create("button", {
  class: "mainbtns",
  id: "cssAlStyle",
  style: { height: "32px", width: "32px", verticalAlign: "middle" },
});
var cssAlStyleText = create(
  "h3",
  { style: { display: "inline" } },
  "Custom CSS + Make profile like Anilist"
);
...
cssButton.onclick = () => {
  if (cssInput.value.length > 1) {
    const cssBase64 = LZString.compressToBase64(
      JSON.stringify([cssInput.value, cssAlStyleEnabled])
    );
...
//Custom Badge
var badgeButton = create(
  "button",
  { class: "mainbtns", id: "custombadge" },
  "Update"
);
let badgeInput = create("input", { class: "badgeInput", id: "badgeInput" });
let badgeColorSelector = create("input", {
  class: "badgeInput",
  id: "badgecolorselector",
  type: "color",
});
let badgeColorLoop = create(
  "button",
  { class: "mainbtns", id: "custombadgeColorLoop" },
  "Rainbow"
);
...
badgeButton.onclick = () => {
  if (badgeInput.value.length > 1) {
    const badgeBase64 = LZString.compressToBase64(
      JSON.stringify([badgeInput.value, badgeColorValue])
    );
...
const createColorInput = () =>
  create("input", { class: "customColorInput", type: "color" });
const customColorButton = create(
  "button",
  { class: "mainbtns", id: "customColorUpdate" },
  "Update"
);
const customColorRemoveButton = create("button", {
  class: "mainbtns fa fa-trash",
  id: "customColorRemove",
});
...
const colorSelectors = Array.from(
  { length: customColorLabels.length },
  (_, index) => {
...
function getSettings() {
  ...
  animeInfoDesignBtn.classList.toggle("btn-active", svar.animeInfoDesign);
  ...
  if (typeof ActHistoryBtn !== "undefined")
    ActHistoryBtn.classList.toggle("btn-active", svar.ActHistory);
  if (!defaultMal) {
...
function createDiv() {
  let listDiv = create(
    "div",
    { class: "malCleanMainContainer" },
    '<div class="malCleanMainHeader"><b>' + stLink.innerText + "</b></div>"
  );
  ...
  listDiv
    .querySelector(".malCleanMainHeader")
    .append(reloadButton, closeButton);
  listDiv.append(
    createListDiv("My Panel", [
      {
        b: buttons["animeInfoBtn"],
        t: "Add info to seasonal anime (hover over anime to make it appear)",
      },
      {
        b: buttons["currentlyWatchingBtn"],
        t: "Show currently watching anime",
      },
      { b: buttons["currentlyReadingBtn"], t: "Show currently reading manga" },
      {
        b: buttons["airingDateBtn"],
        t: "Add next episode countdown to currently watching anime",
      },
      {
        b: buttons["autoAddDateBtn"],
        t: "Auto add start/finish date to watching anime & reading manga",
      },
    ...
    createListDiv("Anime / Manga", [
      {
        b: buttons["animeBgBtn"],
        t: "Add dynamic background color based cover art's color palette",
      },
      ...
      {
        b: buttons["relationFilterBtn"],
        t: "Add filter to replaced relations",
      },
      {
        b: buttons["animeSongsBtn"],
        t: "Replace Anime OP/ED with animethemes.moe",
      },
      {
        b: buttons["editPopupBtn"],
        t: "Replace the edit details with the edit popup",
      },
      {
        b: buttons["animeInfoDesignBtn"],
        t: "Change the design of the Information on the left side.",
      },
    ...
    createListDiv("Profile", [
      ...
      ...(!defaultMal
        ? [
            {
              b: buttons["headerOpacityBtn"],
              t: "Add auto opacity to the header if the user has a custom banner.",
            },
          ]
        : []),
      ...(svar.alstyle
  ...
  customColorsDiv.append(
    customColors,
    customColorButton,
    customColorRemoveButton
  );
  privateProfileDiv.append(privateButton, removePrivateButton);
  hideProfileElDiv.append(
    hideProfileElButton,
    hideProfileElUpdateButton,
    removehideProfileElButton
  );
  customProfileElDiv.append(
    customProfileElUpdateButton,
  ...
  document
    .querySelector("#headerSmall")
    .insertAdjacentElement("afterend", listDiv);
...
(function () {
  ...
  if (
    /\/(forum)\/.?topicid([\w-]+)?\/?/.test(location.href) ||
    /\/(news)\/\d/.test(location.href)
  ) {
    ...
    spoilers.forEach((spoiler) => {
      ...
      hideButton.setAttribute(
        "onclick",
        hideButton.getAttribute("onclick") +
          'this.parentElement.querySelector("iframe.movie").removeAttribute("src")'
      );
    });
  }
  ...
  function on_load() {
    ...
    if (!pfHeader) {
      pfHeader = document.querySelector(
        ".header-profile-dropdown > ul > li:nth-last-child(3)"
      );
    }
    if (pfHeader) {
  ...
  if (svar.currentlyWatching && location.pathname === "/") {
    ...
    async function getWatching() {
      ...
      if (user) {
        const currentlyWatchingDiv = create("article", {
          class: "widget-container left",
        ...
        const html = await fetch(
          "https://myanimelist.net/animelist/" + user + "?status=1"
        )
          ...
          .then((data) => {
            ...
            let list = JSON.parse(
              newDocument
                .querySelector("#list-container > div.list-block > div > table")
                .getAttribute("data-items")
            );
            if (list) {
              document
                .querySelector("#content > div.left-column")
                .prepend(currentlyWatchingDiv);
              processList();
              async function processList() {
                if (svar.airingDate) {
                  ...
                  const queries = idArray.map(
                    (id, index) =>
                      `Media${index}: Media(idMal: ${id}, type: ANIME) {nextAiringEpisode {timeUntilAiring episode}}`
                  );
                  ...
                  async function fetchAnimeData() {
                    ...
                    let d = document.querySelector(
                      "#currently-watching > div > div.widget-header"
                    );
                    if (d) {
                      let e = create(
                        "span",
                        {
                          class: "currently-watching-error",
                          style: { float: "right", display: "inline-block" },
                        },
                        "API Error. Unable to get next episode countdown "
                      );
                      let r = create("i", {
                ...
                for (let x = 0; x < list.length; x++) {
                  ...
                  if (svar.airingDate && infoData) {
                    ...
                    left =
                      ep && airing
                        ? "<div id=" +
                          airing +
                          ' class="airingInfo"><div>Ep ' +
                          ep +
                          "</div>" +
                          "<div>" +
                          (await airingTime(airing)) +
                          "</div></div>"
                        : "";
                    let info = [ep, left];
                    if (info) {
                  ...
                  if (!nextep || !infoData) {
                    nextep =
                      ...
                      list[x].num_watched_episodes +
                      (list[x].anime_num_episodes !== 0
                        ? " / " + list[x].anime_num_episodes
                        : "") +
                      "</div></div>";
                  }
                  let ib = create("i", {
                    class: "fa fa-pen editCurrently",
                    id: list[x].anime_id,
                  });
                  let increaseButton = create("i", {
                    class: "fa fa-plus incButton",
                    id: list[x].anime_id,
                  });
                  ...
                  document
                    .querySelector("#widget-currently-watching ul")
                    .append(wDiv);
                  ib.onclick = async () => {
                ...
                if (svar.airingDate) {
                  let airingDivs = Array.from(
                    document.querySelectorAll(
                      "#widget-currently-watching ul li"
                    )
                  );
                  let airingMainDiv = document.querySelector(
                    "#widget-currently-watching ul"
                  );
                ...
                document
                  .querySelector(
                    "#currently-watching > div > div.widget-header > i"
                  )
                  .remove();
                document.querySelector(
                  "#widget-currently-watching > div.widget-slide-outer > ul"
                ).children.length > 5
                  ? document
                      .querySelector("#current-right")
                ...
                .addEventListener("click", function () {
                  ...
                  if (parseInt(slider.style.marginLeft) < 0) {
                    slider.style.marginLeft =
                      parseInt(slider.style.marginLeft) + slideWidth + "px";
                    document.querySelector(
                      "#widget-currently-watching > div.widget-slide-outer > ul"
                    ).children.length > 5
                      ? document
              ...
              document
                ...
                .addEventListener("click", function () {
                  ...
                  if (
                    ...
                    -slideWidth * (slider.children.length - 5)
                  ) {
                    slider.style.marginLeft =
                      parseInt(slider.style.marginLeft) - slideWidth + "px";
                    document
  ...
  if (svar.currentlyReading && location.pathname === "/") {
    ...
    async function getreading() {
      ...
      let user = $(".header-profile-link").text();
      if (user) {
        const currentlyReadingDiv = create("article", {
          class: "widget-container left",
        ...
        const html = await fetch(
          "https://myanimelist.net/mangalist/" + user + "?status=1"
        )
          .then((response) => response.text())
          .then((data) => {
            var newDocument = new DOMParser().parseFromString(
            ...
            if (list) {
              if (document.querySelector("#currently-watching")) {
                document
                  ...
                  .insertAdjacentElement("afterend", currentlyReadingDiv);
              } else {
                document
                  .querySelector("#content > div.left-column")
                  .prepend(currentlyReadingDiv);
              }
              processList();
              async function processList() {
                for (let x = 0; x < list.length; x++) {
                  let nextchap =
                    ...
                    (list[x].manga_num_chapters !== 0
                      ? " / " + list[x].manga_num_chapters
                      : "") +
                    "</div></div>";
                  let ib = create("i", {
                    class: "fa fa-pen editCurrently",
                  ...
                  document
                    .querySelector("#widget-currently-reading ul")
                    .append(rDiv);
                  ib.onclick = async () => {
                ...
                document
                  .querySelector(
                    "#currently-reading > div > div.widget-header > i"
                  )
                  .remove();
                document.querySelector(
                  "#widget-currently-reading > div.widget-slide-outer > ul"
                ).children.length > 5
                  ? document
                      .querySelector("#current-right-manga")
                      .classList.add("active")
                  : "";
              }
              ...
              document
                .querySelector("#current-left-manga")
                .addEventListener("click", function () {
                  const slider = document.querySelector(
                    ".widget-slide.js-widget-slide.manga"
                  );
                  const slideWidth = slider.children[0].offsetWidth + 12;
                  if (parseInt(slider.style.marginLeft) < 0) {
                    slider.style.marginLeft =
  ...
  if (svar.recentlyAddedAnime && location.pathname === "/") {
    ...
    async function buildRecentlyAddedAnime() {
      let user = $(".header-profile-link").text();
      if (user) {
        const recentlyAddedDiv = create("article", {
          class: "widget-container left  recently-anime",
        ...
        async function getList() {
          ...
          // if error
          let d = document.querySelector(
            "#recently-added-anime > div > div.widget-header"
          );
          if (d) {
            let e = create(
              "span",
        ...
        if (list) {
          if (document.querySelector("#currently-reading")) {
            document
              .querySelector("#currently-reading")
              .insertAdjacentElement("afterend", recentlyAddedDiv);
            ...
            document
              .querySelector("#content > div.left-column")
              .prepend(recentlyAddedDiv);
          }
          buildRecentlyAddedList(list, "#widget-recently-added-anime ul");
          document
            .querySelector(
              "#recently-added-anime > div > div.widget-header > i"
            )
            .remove();
          document.querySelector(
            "#widget-recently-added-anime > div.widget-slide-outer > ul"
          ).children.length > 5
            ? document
                .querySelector("#current-right-recently-added-anime")
                .classList.add("active")
            : "";

          let animeItemsBackup = Array.from(
            document.querySelectorAll(
              "#widget-recently-added-anime ul .btn-anime"
            )
          );
          document.querySelector(
            "#widget-recently-added-anime ul"
          ).style.width =
            138 *
              document.querySelectorAll(
                "#widget-recently-added-anime ul .btn-anime"
          ...
          //Load More Items
          const loadMoreButton = create(
            "a",
            { id: "recently-added-anime-load-more" },
            "Load More"
          );
          const slider = document.querySelector(
            ".widget-slide.js-widget-slide.recent-anime"
          );
          slider.append(loadMoreButton);
          filterRecentlyAdded(animeItemsBackup, ["TV", "Movie"]);
          updateRecentlyAddedSliders(
            slider,
            "#current-left-recently-added-anime",
            "#current-right-recently-added-anime"
          );

          loadMoreButton.addEventListener("click", async function () {
            if (loadMoreButton.innerHTML === "Load More") {
              loadMoreButton.innerHTML =
                '<i class="fa fa-circle-o-notch fa-spin" style="top:2px; position:relative;margin-left:5px;font-size:12px;font-family:FontAwesome"></i>';
              const slider = document.querySelector(
                ".widget-slide.js-widget-slide.recent-anime"
              );
              const selectedType = document
                ...
                .value.split(",");
              let pageCount = document
                .getElementById("recently-added-anime")
                .getAttribute("page");
              ...
              document
                .getElementById("recently-added-anime")
                .setAttribute("page", pageCount);
              animeItemsBackup = Array.from(
                document.querySelectorAll(
                  "#widget-recently-added-anime ul .btn-anime"
                )
              );
          ...
          // Slider Left
          document
            .querySelector("#current-left-recently-added-anime")
            .addEventListener("click", function () {
              const slider = document.querySelector(
                ".widget-slide.js-widget-slide.recent-anime"
              );
  ...
  if (svar.recentlyAddedManga && location.pathname === "/") {
    ...
    async function buildRecentlyAddedManga() {
      ...
      if (user) {
        const recentlyAddedDiv = create("article", {
          class: "widget-container left recently-manga",
          id: "recently-added-manga",
          page: "0",
        });
        recentlyAddedDiv.innerHTML =
          '<div class="widget anime_suggestions left"><div class="widget-header"><span style="float: right;"></span><h2 class="index_h2_seo">' +
        ...
        if (list) {
          if (document.querySelector("#recently-added-anime")) {
            document
              .querySelector("#recently-added-anime")
              .insertAdjacentElement("afterend", recentlyAddedDiv);
          ...
          document
            .querySelector(
              "#recently-added-manga > div > div.widget-header > i"
            )
            .remove();
          document.querySelector(
            "#widget-recently-added-manga > div.widget-slide-outer > ul"
          ).children.length > 5
            ? document
                .querySelector("#current-right-recently-added-manga")
                .classList.add("active")
            : "";

          let mangaItemsBackup = Array.from(
            document.querySelectorAll(
              "#widget-recently-added-manga ul .btn-anime"
            )
          );
          document.querySelector(
            "#widget-recently-added-manga ul"
          ).style.width =
            138 *
              document.querySelectorAll(
                "#widget-recently-added-manga ul .btn-anime"
          ...
          const loadMoreButton = create(
            "a",
            { id: "recently-added-manga-load-more" },
            "Load More"
          );
          const slider = document.querySelector(
            ".widget-slide.js-widget-slide.recent-manga"
          );
          slider.append(loadMoreButton);
          filterRecentlyAdded(mangaItemsBackup, ["Manga"]);
          updateRecentlyAddedSliders(
            slider,
            "#current-left-recently-added-manga",
            "#current-right-recently-added-manga"
          );
          ...
          loadMoreButton.addEventListener("click", async function () {
            if (loadMoreButton.innerHTML === "Load More") {
              loadMoreButton.innerHTML =
                '<i class="fa fa-circle-o-notch fa-spin" style="top:2px; position:relative;margin-left:5px;font-size:12px;font-family:FontAwesome"></i>';
              const selectedType = document
                .getElementById("typeFilterManga")
                .value.split(",");
              let pageCount = document
                .getElementById("recently-added-manga")
                .getAttribute("page");
              pageCount = parseInt(pageCount) + 50;
          ...
          // Slider Left
          document
            .querySelector("#current-left-recently-added-manga")
            .addEventListener("click", function () {
              const slider = document.querySelector(
                ".widget-slide.js-widget-slide.recent-manga"
              );
  ...
  if (/\/(forum)\/.?(topicid|animeid|board)([\w-]+)?\/?/.test(location.href)) {
    ...
    async function getBlockedUsers() {
      const html = await fetch(
        "https://myanimelist.net/editprofile.php?go=privacy"
      )
        .then((response) => response.text())
        .then((data) => {
          let newDocument = new DOMParser().parseFromString(data, "text/html");
          let findUser = newDocument.querySelectorAll(
            "#content > div:nth-child(2) a[href*=profile]"
          );
    ...
    function removeBlockedUsers() {
      ...
      let ForumTopic = document.querySelectorAll(
        "#forumTopics tr[data-topic-id]"
      );
      for (let x = 0; x < ForumTopic.length; x++) {
        for (let y = 0; y < blockedUsers.length; y++) {
          if (
            ForumTopic[x].children[1].children[4].innerText === blockedUsers[y]
          ) {
            ForumTopic[x].remove();
          }
        }
      ...
      let forumReply = document.querySelectorAll(
        ".message-wrapper > div.profile"
      );
      ...
      let forumReplyV = document.querySelectorAll(
        ".messages.replies.parents .message"
      );
      for (let x = 0; x < forumReplyV.length; x++) {
        if (!forumReplyV[x].getAttribute("checked")) {
          for (let y = 0; y < blockedUsers.length; y++) {
            if (
              forumReplyV[x].children[0].children[1].children[0].children[0]
                .innerText === blockedUsers[y]
            ) {
    ...
    if (
      document.querySelectorAll(" .content > div.user > div.item.update").length
    ) {
  ...
  function changeDate(d) {
    let dateData =
      document.querySelectorAll(".message-header > .date").length > 0
        ? document.querySelectorAll(".message-header > .date")
        : document.querySelectorAll(".content > div.user > div.item.update");
    let lastPost = d
      ? d
      : document.querySelectorAll(
          "#forumTopics tr[data-topic-id] td:nth-child(4)"
        );
    if (lastPost) {
      for (let x = 0; x < lastPost.length; x++) {
        let t = d
          ? lastPost[x].innerHTML
          : $(lastPost[x]).find("br").get(0).nextSibling.nodeValue;
        let t2 = d
          ? t
              .replace(/\s{2,}/g, " ")
              .replace(
                /(\w.*\d.*) (\d.*\:\d{2}.*\W.\w)(\sby.*)/gm,
                '<span class ="replyDate">$1 $2</span>$3'
              )
          : t
              .replace(/\s{2,}/g, " ")
              .replace(
                /(\w.*\d.*) (\d.*\:\d{2}.*\W.\w)/gm,
                '<span class ="replyDate">$1</span>'
              )
              .replace(",", " ");
    ...
    let topicDate = Array.prototype.slice
      .call(
        document.querySelectorAll("#forumTopics tr[data-topic-id] .lightLink")
      )
      .concat(
        Array.prototype.slice.call(
          document.querySelectorAll(
            "#forumTopics tr[data-topic-id] td:nth-child(4) span"
          )
        )
      );
    if (d) {
    ...
    for (let x = 0; x < dateData.length; x++) {
      if (!dateData[x].getAttribute("dated")) {
        date = !timeRegex.test(dateData[x].innerText)
          ? dateData[x].innerText + ", 00:00 AM"
          : dateData[x].innerText;
        datenew =
          ...
          date.includes("seconds")
            ? true
            : false;
        date = yearRegex.test(date)
          ? date
          : date.replace(/(\,)/, " " + new Date().getFullYear());
        datenew ? (date = dateData[x].innerText) : date;
        let timestamp = new Date(date).getTime();
        const timestampSeconds = dateData[x].getAttribute("data-time")
          ? dateData[x].getAttribute("data-time")
          : Math.floor(timestamp / 1000);
        dateData[x].title = dateData[x].innerText;
        dateData[x].innerText = datenew
          ? date
  ...
  if (svar.forumDate && location.href === "https://myanimelist.net/forum/") {
    let replyDate = Array.prototype.slice
      .call(document.querySelectorAll("span.date.di-ib"))
      .concat(
        Array.prototype.slice.call(
          document.querySelectorAll("span.information.di-ib")
        )
      );
  ...
  if (
    svar.forumDate &&
    /\/(forum)\/.?(topicid|animeid|board)([\w-]+)?\/?/.test(location.href)
  ) {
    changeDate();
    if (
      document.querySelectorAll(".content > div.user > div.item.update").length
  ...
  if (svar.embed && /\/(forum)\/.?topicid([\w-]+)?\/?/.test(location.href)) {
    const embedCache = localforage.createInstance({
      name: "MalJS",
      storeName: "embed",
    });
    ...
    async function getEmbedData(id, type) {
      ...
      try {
        ...
        if (data && data.data && data.data.status) {
          if (
            data.data.status === "Finished Airing" ||
        ...
        if (imgdata) {
          ...
          const year =
            cached && data.data.year
              ? data.data.year
              : data.data.type !== "Anime" && publishedYear
              ? publishedYear
              : airedYear
              ? airedYear
              : data.data.type === "Anime" && airedYear
              ? airedYear
              : "";
          if (year) {
          ...
          if (data.data.score) {
            detailsArray.push(
              '<span class="score">' +
                " · " +
                Math.floor(data.data.score * 10) +
                "%" +
                "</span>"
            );
          }
          const detailsArrayLast =
            detailsArray.length > 0
              ? detailsArray[detailsArray.length - 1].toString()
              : "";
          details.innerHTML = detailsArrayLast.includes("score")
            ? detailsArray.slice(0, -1).join(" · ") + detailsArray.slice(-1)
            : detailsArray.join(" · ");
          ...
          if (
            data.data.type &&
            ["Manga", "Manhwa", "Novel"].includes(data.data.type)
          ) {
    ...
    async function embedload() {
      const forumHeader =
        document.querySelector("h1.forum_locheader")?.innerText;
      const headerRegex = /\w+\s\d{4}\sPreview/gm;
      const c =
        document.querySelectorAll(".message-wrapper > div.content").length > 0
          ? document.querySelectorAll(".message-wrapper > div.content")
          : document.querySelectorAll(".forum.thread .message");
      for (let x = 0; x < c.length; x++) {
        let content = c[x].innerHTML;
        content = content
          .replace(
            /(http:\/\/|https:\/\/)(myanimelist\.net\/(anime|manga)\.php\?id=)([0-9]+)/gm,
            "https://myanimelist.net/$2/$3"
          )
          .replace(
            /(<a href="\b(http:\/\/|https:\/\/)(myanimelist\.net\/(anime|manga)\/[0-9]+))/gm,
            " $1"
          );
        let matches = content.match(
          /<a href="\b(http:\/\/|https:\/\/)(myanimelist\.net\/(anime|manga)\/)([0-9]+)([^"'<]+)(?=".\w)/gm
        );
        matches = matches
          ? matches.filter((link) => !link.includes("/video"))
          : matches;
  ...
  if (/\/(profile)\/.?([\w]+)?\/?/.test(current)) {
    ...
    async function imgLoad() {
      ...
      if (userimg && userimg.src) {
        set(0, userimg, {
          sa: { 0: "position: relative;opacity:1!important;" },
        });
    ...
    async function startCustomProfile() {
      ...
      if (customcss && customcss.constructor === Array && customcss[1]) {
        ...
        svar.alstyle = false;
        await applyAl();
      } else if (customAlStyleFounded || svar.alstyle === true) {
        svar.alstyle = true;
        await applyAl();
      } else {
        pfloading.remove();
    ...
    if (
      !/\/profile\/.*\/\w/gm.test(current) &&
      username !== $(".header-profile-link").text() &&
      $(".header-profile-link").text() !== "" &&
      $(".header-profile-link").text() !== "MALnewbie"
    ) {
    ...
    if (
      ...
      shadow.setAttribute(
        "style",
        "background: linear-gradient(180deg,rgba(6,13,34,0) 40%,rgba(6,13,34,.6));height: 100%;left: 0;position: absolute;top: 0;width: 100%;"
      );
    ...
    async function buildCustomElements(data) {
      ...
      let customElContent = create("div", {
        class: "customProfileEls",
        id: "customProfileEls",
      });
      let customElContentRight = create("div", {
        class: "customProfileEls right",
        id: "customProfileEls",
      });
      let sortItem1 = null;
      let sortItem2 = null;
      ...
      favarray.forEach((arr, index) => {
        arr.forEach((item) => {
          ...
          const customElContainer = create("div", {
            class: "custom-el-container",
          });
          ...
          <div class="${
            svar.alstyle ? "custom-el-inner" : "custom-el-inner notAl"
          }">${item.content}</div>
          `;
          if (isRight) {
        ...
        element.addEventListener("click", function () {
          ...
          if (sortItem1 === null) {
            ...
            $(this)
              .parent()
              .parent()
              .children(".custom-el-container")
              .children(".sortCustomEl")
              .removeClass("hidden");
            $(this)
              .parent()
              .parent()
          ...
          function replaceCustomEls() {
            const sortItem1compressedBase64 = LZString.compressToBase64(
              JSON.stringify(favarray[sortItem1])
            );
            const sortItem1base64url = sortItem1compressedBase64.replace(
              /\//g,
              "_"
            );
            const sortItem2compressedBase64 = LZString.compressToBase64(
              JSON.stringify(favarray[sortItem2])
            );
            const sortItem2base64url = sortItem2compressedBase64.replace(
              /\//g,
              "_"
            );
            editAboutPopup(
              [sortItem1base64url, sortItem2base64url],
              "replaceCustomEl"
            );
            sortItem1 = null;
            sortItem2 = null;
      ...
      $(".editCustomEl").on("click", function () {
        ...
        const content = $(this)
          .nextUntil(".custom-el-inner")
          .next(".custom-el-inner")
          .html();
        const compressedBase64 = LZString.compressToBase64(
          JSON.stringify(favarray[$(this).attr("order")])
        );
    ...
    async function buildFavSongs(data) {
      ...
      let sortItem2 = null;
      if (svar.alstyle) {
        $(FavContent).insertBefore(
          $(
            "#content > div > div.container-left > div li.icon-statistics.link"
          ).parent()
        );
      } else {
        $("#content > div > div.container-right > h2")
          .nextUntil(".user-comments")
          .wrapAll("<div class='favContainer' id='user-def-favs'></div>");
      ...
      favarray.forEach((arr, index) => {
        arr.forEach((item) => {
          const favSongContainer = create("div", {
            class: "fav-theme-container",
            type: item.themeType,
          });
          favSongContainer.innerHTML = `
            ...
            }" class="anime-image" alt="${item.animeTitle}">`}</a>
            <div class="fav-theme-header">
            <h2>${item.animeTitle}</h2>
            <a class="favThemeSongTitle" style="cursor:pointer">${item.songTitle.replace(
              /(\(eps \d.*\))/,
              ""
            )}</a>
            </div></div>
      ...
      const animeContainers = favThemes.querySelectorAll(
        ".fav-theme-container"
      );
        ...
        element.addEventListener("click", function () {
          ...
          if (sortItem1 === null) {
            ...
            $(this)
              .parent()
              .parent()
              .children(".fav-theme-container")
              .children(".sortFavSong")
              .removeClass("hidden");
            $(this)
              .parent()
              .parent()
    ...
    async function applyCustomColors(customcolors) {
      ...
      .profile .user-statistics .stats-graph .graph.plan_to_watch {background:${
        customcolors[4]
      }!important}
      ...
      .profile .user-statistics .stats-graph .graph.reading {background:${
        customcolors[5]
      }!important}
      ...
      .profile .user-statistics .stats-graph .graph.manga.completed {background:${
        customcolors[6]
      }!important}
      ...
      .profile .user-statistics .stats-graph .graph.manga.on_hold {background:${
        customcolors[7]
      }!important}
      ...
      .profile .user-statistics .stats-graph .graph.manga.dropped {background:${
        customcolors[8]
      }!important}
      ...
      .profile .user-statistics .stats-graph .graph.plan_to_read {background:${
        customcolors[9]
      }!important}
      .profile #profile-stats-anime-genre-table table .list-item .data.ratio > div .mal-progress .mal-progress-bar.primary{background:${
        customcolors[10]
      }!important}
      #profile-stats-anime-score-dist g.amcharts-Sprite-group.amcharts-Container-group[fill="#338543"]{fill:${
        customcolors[0]
      }!important;}
      #profile-stats-anime-score-dist g.amcharts-Sprite-group.amcharts-Container-group[fill="#2d4276"]{fill:${
        customcolors[1]
      }!important;}
      #profile-stats-anime-score-dist g.amcharts-Sprite-group.amcharts-Container-group[fill="#c9a31f"]{fill:${
        customcolors[2]
      }!important;}
      #profile-stats-anime-score-dist g.amcharts-Sprite-group.amcharts-Container-group[fill="#832f30"]{fill:${
        customcolors[3]
      }!important;}
      #profile-stats-manga-score-dist g.amcharts-Sprite-group.amcharts-Container-group[fill="#338543"]{fill:${
        customcolors[5]
      }!important;}
      #profile-stats-manga-score-dist g.amcharts-Sprite-group.amcharts-Container-group[fill="#2d4276"]{fill:${
        customcolors[6]
      }!important;}
      #profile-stats-manga-score-dist g.amcharts-Sprite-group.amcharts-Container-group[fill="#c9a31f"]{fill:${
        customcolors[7]
      }!important;}
      #profile-stats-manga-score-dist g.amcharts-Sprite-group.amcharts-Container-group[fill="#832f30"]{fill:${
        customcolors[8]
      }!important;}
      #profile-stats-anime-score-dist g.amcharts-Sprite-group.amcharts-Container-group span[style="color: #338543;"]{color:${
        customcolors[0]
      }!important}
      #profile-stats-anime-score-dist g.amcharts-Sprite-group.amcharts-Container-group span[style="color: #2d4276;"]{color:${
        customcolors[1]
      }!important}
      #profile-stats-anime-score-dist g.amcharts-Sprite-group.amcharts-Container-group span[style="color: #c9a31f;"]{color:${
        customcolors[2]
      }!important}
      #profile-stats-anime-score-dist g.amcharts-Sprite-group.amcharts-Container-group span[style="color: #832f30;"]{color:${
        customcolors[3]
      }!important}
      #profile-stats-manga-score-dist g.amcharts-Sprite-group.amcharts-Container-group span[style="color: #338543;"]{color:${
        customcolors[5]
      }!important}
      #profile-stats-manga-score-dist g.amcharts-Sprite-group.amcharts-Container-group span[style="color: #2d4276;"]{color:${
        customcolors[6]
      }!important}
      #profile-stats-manga-score-dist g.amcharts-Sprite-group.amcharts-Container-group span[style="color: #c9a31f;"]{color:${
        customcolors[7]
      }!important}
      #profile-stats-manga-score-dist g.amcharts-Sprite-group.amcharts-Container-group span[style="color: #832f30;"]{color:${
        customcolors[8]
      }!important}
      .profile #myanimelist li.btn-fav .title.fs10{color:${
        customcolors[10]
      }!important}
    ...
    async function findCustomAbout() {
      const aboutSection = document.querySelector(
        ".user-profile-about.js-truncate-outer"
      );
      const processAboutSection = (aboutContent) => {
        ...
        const privateProfileMatch = aboutContent.match(
          profileRegex.privateProfile
        );
        const hideProfileElMatch = aboutContent.match(
          profileRegex.hideProfileEl
        );
        ...
        if (pfMatch) {
          ...
          if (pfData !== "...") {
            ...
            document
              .querySelector(".user-image.mb8 > img")
              .setAttribute("src", custompf);
          }
        }
        if (bgMatch) {
        ...
        if (colorMatch) {
          ...
          if (colorData !== "...") {
            ...
            customcolors = JSON.parse(
              LZString.decompressFromBase64(colorBase64Url)
            );
            applyCustomColors(customcolors);
          }
        }
        if (privateProfileMatch) {
          const privateData = privateProfileMatch[0].replace(
            profileRegex.privateProfile,
            "$2"
          );
          if (privateData !== "...") {
            let privateBase64Url = privateData.replace(/_/g, "/");
            privateProfile = JSON.parse(
              LZString.decompressFromBase64(privateBase64Url)
            );
        ...
        if (hideProfileElMatch) {
          const hideProfileElData = hideProfileElMatch[0].replace(
            profileRegex.hideProfileEl,
            "$2"
          );
          if (hideProfileElData !== "...") {
            let profileElBase64Url = hideProfileElData.replace(/_/g, "/");
            hiddenProfileElements = JSON.parse(
              LZString.decompressFromBase64(profileElBase64Url)
            );
            applyHiddenDivs();
          }
        }
        if (badgeMatch) {
          ...
          if (badgeData !== "...") {
            ...
            custombadge = JSON.parse(
              LZString.decompressFromBase64(badgeBase64Url)
            );
      ...
      if (aboutSection && aboutSection.innerHTML.match(profileRegex.malClean)) {
        ...
        try {
          ...
          if (
            userProfileAbout &&
            userProfileAbout.innerHTML.match(profileRegex.malClean)
          ) {
      ...
      if (!settingsFounded) {
        try {
          const rssUrl =
            "https://myanimelist.net/rss.php?type=blog&u=" + username;
    ...
    async function applyAl() {
      ...
      if (svar.alstyle) {
        ...
        document.body.style.setProperty(
          "--color-foreground",
          "var(--color-foregroundOP)",
          "important"
        );
        document.body.style.setProperty(
          "--color-foreground2",
          "var(--color-foregroundOP2)",
          "important"
        );
        ...
        if (svar.ActHistory) {
          const titleImageMap = {};
          let historyMain = create("div", {
            class: "user-history-main",
            id: "user-history-div",
          });
          if (document.querySelector("#statistics")) {
            document
              .querySelector("#statistics")
              .insertAdjacentElement("beforeend", historyMain);
          }
          async function gethistory(l, item) {
            ...
            if (!l) {
              const html = await fetch(
                ...
                .then(async (data) => {
                  ...
                  item = newDocument.querySelectorAll(
                    "#content > div.history_content_wrapper > table > tbody > tr > td.borderClass:first-child"
                  );
                });
            }
            ...
            for (let x = c; x < length; x++) {
              ...
              date = item[x].parentElement.children[1].innerText
                .split("Edit")
                .join("");
              datenew =
                date.includes("Yesterday") ||
              ...
              let historydate = create(
                "div",
                { class: "user-history-date", title: date },
                datenew ? date : nativeTimeElement(timestampSeconds)
              );
              let apiUrl = `https://api.jikan.moe/v4/anime/${id}`;
              if (type === "anime") {
                name.innerHTML =
        ...
        container.setAttribute(
          "style",
          "margin: 0 auto;min-width: 320px;max-width: 1240px;left: -40px;position: relative;height: 100%;"
        );
        if (!custombg) {
          banner.setAttribute(
            "style",
            "background-color: var(--color-foreground);background-position: 50% 35%; background-repeat: no-repeat;background-size: cover;height: 330px;position: relative;"
          );
        } else {
        ...
        document
          .querySelector("#myanimelist")
          .setAttribute("style", "min-width: 1240px;width:100%");
        set(1, "#myanimelist .wrapper", {
          sa: { 0: "width:100%;display:table" },
        });
        document
        ...
        if (
          ...
          $(headerRight)
            .wrapAll("<div class='profileRightActions'></div>")
            .after(blockU);
        }
        ...
        if (
          set(0, about, {
            sa: {
              0: "margin-bottom: 20px;width: auto;background: var(--color-foreground);padding: 10px;border-radius: var(--br);",
            },
          })
        ) {
          document
            .querySelector(
              "#content > div > div.container-left > div > ul.user-status.border-top.pb8.mb4"
            )
            .insertAdjacentElement("beforebegin", about);
        }
        if (
          set(0, modernabout, {
            sa: {
          ...
          document
            ...
            .insertAdjacentElement("beforebegin", modernabout);
        ...
        if (about || modernabout) {
          if (
            set(1, ".user-profile h1:first-child", {
              sa: { 0: "position: absolute;top: 50px;right: 0;" },
            })
          ) {
            banner.append(
              document.querySelector(".user-profile h1:first-child")
            );
          }
        ...
        document
          ...
          .setAttribute("style", "top: 99px;left: 99px;position: relative;");
        set(1, ".user-statistics-stats.mt16", {
          sa: { 0: "margin-top:8px!important" },
        });
        set(1, ".user-image .btn-detail-add-picture", {
          sa: {
            0: "display: flex;flex-direction: column;justify-content: center;",
        ...
        avatar.setAttribute(
          "style",
          "display: flex;height: inherit;align-items: flex-end;position: relative;width:500px;"
        );
        name.css({
          "font-size": "2rem",
          "font-weight": "800",
        ...
        $(".user-function.mb8")
          .children(".icon-request")
          .addClass("maljsNavBtn");
        $(".user-function.mb8")
        ...
        $(".user-function.mb8")
          ...
          .addClass("maljsNavBtn");
        if (
          $(".user-function.mb8").attr("class") ===
            "user-function mb8 display-none" &&
          $(".maljsProfileBadge").length
        ) {
        ...
        if (
          set(
            1,
            ".user-profile-about.js-truncate-outer .btn-truncate.js-btn-truncate",
            { sa: { 0: "display:none" } }
          )
        ) {
          set(
        ...
        set(1, ".user-function.mb8", {
          sa: {
            0: "position: relative;left: 96.5%;top: -60px;display: flex;width: 100px;font-size: 1rem;justify-content: flex-end;gap: 6px;",
          },
        });
        set(2, ".user-function.mb8 a", {
          sal: { 0: "border:none!important;box-shadow:none!important" },
        });
        set(2, ".user-function.mb8 span", {
          sal: { 0: "border:none!important;box-shadow:none!important" },
        });
        ...
        if (
          set(1, ".content-container", {
            sa: {
              0: "display: grid!important;grid-template-columns: 33% auto;margin-top: 20px;justify-content: center;",
            },
          })
        ) {
          set(1, ".container-left", { sa: { 0: "width:auto" } });
          set(1, ".container-right", {
            sa: { 0: "width:auto;min-width:800px" },
          });
        }
        ...
        if (
          ...
          set(1, "#content > table > tbody > tr > td.pl8", {
            sa: { 0: "width: auto;position:relative;min-width:800px" },
          });
        }
        ...
        if (s) {
          ...
          let mangaUpdates = document.querySelector(
            "#statistics .updates.manga"
          );
          let animeStats = document.querySelector("#statistics .stats.anime");
          let animeUpdates = document.querySelector(
            "#statistics .updates.anime"
          );
          ...
          if (
            animeStats.children[1].innerText ===
              "Days: 0.0\tMean Score: 0.00" &&
            mangaStats.children[1].innerText === "Days: 0.0\tMean Score: 0.00"
          ) {
            ...
            if (
              animeStats &&
              animeStats.children[1].innerText === "Days: 0.0\tMean Score: 0.00"
            ) {
              animeStats.remove();
              animeUpdates.remove();
              if (
                mangaStats.parentElement &&
                mangaStats.children[1].innerText !==
                  "Days: 0.0\tMean Score: 0.00"
              ) {
        ...
        if ($(".user-button.clearfix.mb12").length) {
          ...
          $(".user-button.clearfix.mb12").after(
            favs,
            favs2,
            favs3,
            favs4,
        ...
        $(".favs").each(function (index) {
          $(this)
            ...
            .addBack()
            .wrapAll(
              "<div class='user-favs' id='fav-" + index + "-div'></div>"
            );
        });
        ...
        let collection = Array.from(userFavs).concat(
          Array.from(userBadges),
          Array.from(userFriends)
        );
        for (let btnFav of collection) {
          ...
          if (btnFav.attributes.title) {
            btnFav.setAttribute(
              "data-title",
              btnFav.attributes.title.textContent
            );
          ...
          if (title) {
            ...
            btnFav.prepend(tt);
            btnFav.tagName === "A" ||
            (btnFav.classList[0] && btnFav.classList[0] === "user-badge")
              ? (tt.style.marginTop = "-5px")
              : "";
        ...
        if (
          !$(".profile .user-profile").length &&
          $("#content > table > tbody > tr > td.profile_leftcell").length
        ) {
          document
            .querySelector(
              "#content > table > tbody > tr > td.profile_leftcell"
            )
            .classList.add("profile");
        }
        set(1, ".container-right > .btn-favmore", { r: { 0: 0 } });
        set(2, ".profile .user-profile h5", {
          sal: { 0: "font-size: 11px;margin-bottom: 8px;margin-left: 2px;" },
        });
        set(2, ".container-left h4", {
          sal: { 0: "font-size: 11px;margin-left: 2px;" },
        });
        //Remove Favorite Count
        const favHeader = document.querySelectorAll(
          ".profile .user-profile .user-favs h5"
        );
        ...
        nav.setAttribute(
          "style",
          "z-index: 3;position: relative;background: #000;text-align: center;background-color: var(--color-foreground) !important;"
        );
        let navel = document.querySelectorAll(
          "#navbar #horiznav_nav > ul > li > a"
        );
        ...
        set(0, navel, {
          sal: {
            0: "margin: 0 30px;font-size: .9rem;box-shadow: none!important;",
          },
        });
      } else {
        pfloading.remove();
    ...
    async function changeForeground(color) {
      if (svar.alstyle) {
        ...
        $("style:contains(--fg:)").html("");
        let customColors = `
        :root,body {--fg: ${color}!important;--color-background: ${tinycolor(
          color
        ).darken(3)}!important;--color-backgroundo: ${tinycolor(color)
          .setAlpha(0.8)
          .toRgbString()}!important;
        --color-foreground: ${color}!important;--color-foreground2: ${tinycolor(
          color
        ).brighten(3)}!important;--color-foreground3: ${tinycolor(
          color
        ).brighten(4)}!important;
        --color-foregroundOP: ${color}!important;--color-foregroundOP2: ${tinycolor(
          color
        ).brighten(3)}!important;--color-foreground4: ${tinycolor(
          color
        ).brighten(6)}!important;
    ...
    let contLeft = $(".container-left").length
      ? $(".container-left")
      : $("#content > table > tbody > tr td[valign='top']:nth-child(1)");
    let contRight = $(".container-right").length
      ? $(".container-right")
      : $("#content > table > tbody > tr td[valign='top']:nth-child(2)");
    if (svar.replaceList) {
      //Anime List Button
      const animeListButton = document.querySelector(
        "a.btn-profile-submit.fl-l"
      );
      if (animeListButton) {
        ...
        animeListButton.onclick = async () => {
          ...
          $(
            "#content > div > div.container-right > div.favmore > h5:nth-child(1)"
          ).hide();
          $(
            "#content > div > div.container-right > div.favmore > h5:nth-child(3)"
          ).hide();
          getAnimeList();
        };
      }
      //Manga List Button
      const mangaListButton = document.querySelector(
        "a.btn-profile-submit.fl-r"
      );
      if (mangaListButton) {
    ...
    function createEntryRow(animeData) {
      ...
      let section = document.getElementById(
        `status-section-${animeData.status}`
      );
      if (!section) {
      ...
      if (animeData.notes) {
        ...
        const titleNoteIcon = create("span", {
          class: "title-note fa-sticky-note",
        });
      ...
      const progressDiv = create(
        "div",
        { class: "progress" },
        animeData.progress +
          (animeData.progressEnd ? "/" + animeData.progressEnd : "")
      );
      ...
      entryRow.setAttribute(
        "season",
        animeData.season ? JSON.stringify(animeData.season) : ""
      );
      entryRow.setAttribute("tags", animeData.tags ? animeData.tags : "");
      entryRow.setAttribute(
        "startDate",
        animeData.startDate ? animeData.startDate : ""
      );
      entryRow.setAttribute(
        "finishDate",
        animeData.finishDate ? animeData.finishDate : ""
      );
      entryRow.setAttribute(
        "createdAt",
        animeData.createdAt ? JSON.stringify(animeData.createdAt) : ""
      );
      entryRow.setAttribute(
        "updatedAt",
        animeData.updatedAt ? JSON.stringify(animeData.updatedAt) : ""
      );
      entryRow.setAttribute(
        "progress",
        animeData.progress ? JSON.stringify(animeData.progress) : "0"
      );
    }
    ...
    async function fetchWithRetry(url, timeout = 15000, retries = 5) {
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          ...
          if (attempt < retries) {
            $(".listLoading").html(
              `Retrying (${attempt}/${retries})... <i class="fa fa-circle-o-notch fa-spin" style="top:2px; position:relative;margin-left:5px;font-family:FontAwesome"></i>`
            );
      ...
      while (true) {
        try {
          const response = await fetchWithRetry(
            `https://myanimelist.net/${
              isManga ? "mangalist/" + username : "animelist/" + username
            }/load.json?offset=${offset}&status=7`
          );
          const data = await response.json();
          if (data.length === 0) {
            break;
    ...
    async function getAnimeList(type) {
      ...
      const fetchUrl = isManga
        ? "https://myanimelist.net/mangalist/" + username + "?status=7"
        : "https://myanimelist.net/animelist/" + username + "?status=7";
      ...
      if (svar.alstyle) {
        const contentDiv = document.querySelector("#content > div")
          ...
          : document.querySelector("#content > table > tbody > tr");
        ...
        document.querySelector(
          "#contentWrapper > div > h1.h1"
        ).style.marginBottom = "25px";
      } else if (
        svar.profileHeader &&
        document.querySelector("#contentWrapper > div")
      ) {
        document.querySelector("#contentWrapper > div").style.marginBottom =
          "25px";
      }
      ...
      listFilter.innerHTML =
        '<label for="filter-input"></label><input type="text" id="filter-input" placeholder="Filter"><h3>Lists</h3>';
      const goBack = create("a", {
        class: "filterLists-back fa fa-arrow-left",
      });
      goBack.onclick = () => {
      ...
      $(listFilter).prepend(
        $("<h3>", {
          text: isManga ? "Manga List" : "Anime List",
          css: { marginTop: 0 },
        })
      );
      ...
      const listsDivContainer = create("div", {
        class: "filterListsDivContainer",
      });
      ...
      $(".genre-filter").click(function () {
        ...
        entries.forEach((entry) => {
          ...
          const isVisible =
            selectedGenres.every((genre) => entryGenres.includes(genre)) ||
      ...
      const yearFilterClear = create("i", {
        class: "year-filter-clear fa fa-close",
      });
      yearFilter.innerHTML =
        '<div class="year-filter-slider-container">' +
        '<input type="range" id="year-filter-slider" min="1917" max="' +
        yearFilterThisYear +
        '"value="' +
        yearFilterThisYear +
        '" step="1"><span id="year-filter-label">' +
        yearFilterThisYear +
        "</span></div>";
      ...
      const tagsContainer = create("div", {
        class: "filterList_TagsContainer",
      });
      const tagsContainerClear = create("i", {
        class: "tags-container-clear fa fa-close",
      });
      ...
      if (username !== $(".header-profile-link").text()) {
        ...
        let compareUrl = isManga
          ? "https://myanimelist.net/sharedmanga.php?u1=" +
            username +
            "&u2=" +
            $(".header-profile-link").text()
          : "https://myanimelist.net/sharedanime.php?u1=" +
            username +
            "&u2=" +
            $(".header-profile-link").text();
      ...
      buttonDraw3x3.onclick = function () {
        if (!document.querySelector(".maljsDisplayBox")) {
          ...
          let col_input = AdvancedCreate(
            "input",
            "maljsNativeInput",
            false,
            displayBox
          );
          let col_label = AdvancedCreate(
            "span",
            false,
            "columns",
            displayBox,
          ...
          col_input.step = 1;
          col_input.min = 0;
          let row_input = AdvancedCreate(
            "input",
            "maljsNativeInput",
            false,
            displayBox
          );
          let row_label = AdvancedCreate(
            "span",
            false,
            "rows",
            displayBox,
          ...
          row_input.step = 1;
          row_input.min = 0;
          let margin_input = AdvancedCreate(
            "input",
            "maljsNativeInput",
            false,
            displayBox
          );
          let margin_label = AdvancedCreate(
            "span",
            false,
            "spacing (px)",
            displayBox,
          ...
          margin_input.min = 0;
          let width_input = AdvancedCreate(
            "input",
            "maljsNativeInput",
            false,
            displayBox
          );
          let width_label = AdvancedCreate(
            "span",
            false,
            "image width (px)",
            displayBox,
          ...
          width_input.min = 0;
          let height_input = AdvancedCreate(
            "input",
            "maljsNativeInput",
            false,
            displayBox
          );
          let height_label = AdvancedCreate(
            "span",
            false,
            "image height (px)",
            displayBox,
          ...
          height_input.min = 0;
          let fitMode = AdvancedCreate(
            "select",
            "maljsNativeInput",
            false,
            displayBox
          );
          let fitMode_label = AdvancedCreate(
            "span",
            false,
            "image fitting",
            displayBox,
          ...
          let recipe = AdvancedCreate(
            "p",
            false,
            "Click 9 media entries, then save the image below",
            displayBox
          );
          let linkList = [];
          ...
          displayBox.parentNode.querySelector(".maljsDisplayBoxClose").onclick =
            function () {
          ...
          let finalCanvas = AdvancedCreate(
            "canvas",
            false,
            false,
            displayBox,
            "max-height: 60%;max-width: 90%"
          );
          ...
          let updateConfig = function () {
            ...
            mode = fitMode.value;
            displayBox.parentNode.querySelector(
              ".maljsDisplayBoxTitle"
            ).textContent = columns + "x" + rows + " Maker";
            recipe.innerText =
              "Click " +
              rows * columns +
              " media entries, then save the image below";
            updateDrawing();
          };
            ...
            cardList.forEach((card) => {
              card.onclick = function () {
                if (this.draw3x3selected) {
                  ...
                  let val = this.querySelector(".cover .image")
    ...
    async function mutualFriends() {
      ...
      const friendsHeader = document.querySelector(
        ".boxlist-container.friend.mb16"
      );
      const mutualsButton = create(
        "a",
        {
          class: "mal-btn",
      ...
      mutualsButton.addEventListener("click", async function () {
        if ($(mutualsButton).text() !== "Loading..") {
          ...
          try {
            ...
            const userFriendsBoxes =
              document.querySelectorAll(".boxlist.col-3");
            mutualsButton.classList[1] === "active"
              ? $(mutualsButton).css({
                  backgroundColor: "var(--color-foreground2)",
                })
              : $(mutualsButton).css({
    ...
    if (
      /\/profile\/.*\/friends/gm.test(current) &&
      username !== $(".header-profile-link").text()
    ) {
      mutualFriends();
    }
    ...
    $(".user-friends.pt4.pb12")
      .prev()
      .addBack()
      .wrapAll("<div id='user-friends-div'></div>");
    $(".user-badges")
      .prev()
      .addBack()
      .wrapAll("<div id='user-badges-div'></div>");
    $(".user-profile-sns")
      .has('a:contains("Recent")')
      .prev()
      .addBack()
      .wrapAll("<div id='user-rss-feed-div'></div>");
    $('.user-profile-sns:not(:contains("Recent"))')
      .prev()
      .addBack()
      .wrapAll("<div id='user-links-div'></div>");
    ...
    $(".user-status:contains(Forum Posts)").attr(
      "id",
      "user-status-counts-div"
    );
  ...
  if (svar.headerSlide || svar.headerOpacity) {
    ...
    if (header && menu) {
      ...
      header.style.transition =
        "top 0.3s ease-in-out, background 0.3s ease-in-out";

      window.addEventListener("scroll", () => {
        //Seasonal Anime Nav Fix
        if (/\/(anime)\/(season).?([\w]+)?\/?/.test(location.pathname)) {
          if (!seasonalNav) {
            seasonalNav = document.querySelector(
              "#content > div.navi-seasonal.js-navi-seasonal.fixed"
            );
          }
  ...
  if (/\/(character)\/.?([\w-]+)?\/?/.test(current)) {
    ...
    let fav = document.querySelector(
      "#content > table > tbody > tr > td.borderClass"
    );
    match.innerText = fav.innerText.match(regex);
    fav.innerHTML = fav.innerHTML.replace(regex, "");
    if (match) {
      document
        .querySelector("#v-favorite")
        .insertAdjacentElement("beforebegin", match);
    }
    if (/\/(clubs)/.test(current) || /\/(pics)/.test(current)) {
      ...
      $('div:contains("Voice Actors"):last')
        ...
        .before(
          '<h2 class="VoiceActorsHeader"style="margin-bottom: -10px;margin-top: 10px;">Voice Actors</h2>'
        );
      ...
      $('h2:contains("Recent Featured Articles"):last')
        .addClass("RecentFeaturedArticlesDiv")
        .append($(".RecentFeaturedArticlesDiv").next());
      ...
      $(".RecentFeaturedArticlesDiv")
        .children()
        .children()
        .css("width", "99%")
        .children()
        .css("borderRadius", "var(--br)");
      let main = document.querySelector(
        "#content > table > tbody > tr > td:nth-child(2)"
      );
      ...
      for (let x = 0; x < spofix.length; x++) {
        spofix[x].setAttribute(
          "onclick",
          "this.parentNode.style.display='none';this.parentNode.previousElementSibling.style.display='inline-block';"
        );
      }
  ...
  if (
    ...
    !/\/(anime|manga)\/producer|genre|magazine|adapted\/.?([\w-]+)?\/?/.test(
      current
    ) &&
    ...
    if ($(".AlternativeTitlesDiv").length) {
      if (
        $("a.js-anime-toggle-alternative-title-button").length > 0 ||
        $("a.js-manga-toggle-alternative-title-button").length > 0
      ) {
        $(".AlternativeTitlesDiv")
          .nextUntil("a")
          .addClass("spaceit-shadow-end")
          .addClass("mb8");
      } else {
        $(".AlternativeTitlesDiv")
          .nextUntil("br")
      ...
      document
        .querySelector(".AlternativeTitlesDiv")
        .nextElementSibling.setAttribute("style", "margin-bottom:4px");
    ...
    if (document.querySelector(".js-alternative-titles.hide")) {
      document
        .querySelector(".js-alternative-titles.hide")
        .setAttribute("style", "border-radius:var(--br);overflow:hidden");
    }
    if ($(".InformationDiv").length) {
      $(".InformationDiv")
        .nextUntil("br")
        .not("h2")
        .attr("style", "background:0!important")
        .addBack()
        .wrapAll("<div class='spaceit-shadow-end-div'></div>");
    }
    ...
    if ($(".ResourcesDiv").length) {
      ...
      document
        ...
        .previousElementSibling.previousElementSibling.setAttribute(
          "style",
          "border-bottom-left-radius:var(--br);border-bottom-right-radius:var(--br)"
        );
      document.querySelector(
        ".ResourcesDiv"
      ).nextElementSibling.style.borderRadius = "var(--br)";
    }
    if ($(".StreamingPlatformsDiv").length) {
      $(".StreamingPlatformsDiv")
        .next(".pb16.broadcasts")
        .attr("style", "padding-bottom: 12px!important");
    ...
    if ($(".InformationDiv").length > 0) {
      let InformationAired = $(".InformationDiv")
        .next()
        .children()
        .children('.dark_text:contains("Aired")');
      if (InformationAired.length > 0) {
        InformationAired = InformationAired.parent()[0].childNodes[3]
          ? InformationAired.parent()[0].childNodes[3]
          : InformationAired.parent()[0].childNodes[2];
        InformationAired.nodeValue = InformationAired.nodeValue.replace(
          "to ?",
          ""
        );
      }
    ...
    async function getAiringTime() {
      if ($(".InformationDiv").length > 0) {
        let InformationAiring = $(".InformationDiv")
          .next()
          .children()
          .children('.dark_text:contains("Status")')
          .parent();
        if (InformationAiring.length > 0) {
          ...
          if (InformationAiring === "Currently Airing") {
            ...
            if (AiringData?.data.Media) {
              const AiringEp = AiringData.data.Media.nextAiringEpisode
                ? AiringData.data.Media.nextAiringEpisode.episode
                : "";
              const AiringTime = AiringData.data.Media.nextAiringEpisode
                ? AiringData.data.Media.nextAiringEpisode.timeUntilAiring
                : "";
              const AiringInfo =
                AiringEp && AiringTime
                  ? '<div class="spaceit_pad"><span class="dark_text">' +
                    (svar.animeInfoDesign ? "Airing" : "Airing: ") +
                    "</span>" +
                    (svar.animeInfoDesign ? "<br>" : "") +
                    "<a>Ep " +
                    AiringEp +
                    ": " +
                    (await airingTime(AiringTime)) +
                    "</a></div>"
                  : "";
              if (AiringInfo) {
                $(".InformationDiv")
    ...
    if (svar.animeBanner) {
      ...
      async function getBannerImage() {
        ...
        const BannerLocalForage = localforage.createInstance({
          name: "MalJS",
          storeName: "banner",
        });
        const BannerCache = await BannerLocalForage.getItem(
          entryId + "-" + entryType
        );
        if (BannerCache) {
          ...
          if (bannerData?.data.Media && bannerData.data.Media.bannerImage) {
            ...
            bannerData = await BannerLocalForage.getItem(
              entryId + "-" + entryType
            );
          } else {
            bannerData = null;
        ...
        if (bannerData && bannerData?.bannerImage && bannerTarget) {
          ...
          const bannerShadowColor = [
            bgColor.setAlpha(0.1).toRgbString(),
            bgColor.setAlpha(0.0).toRgbString(),
            bgColor.setAlpha(0.6).toRgbString(),
          ];
    ...
    if (svar.animeTag) {
      ...
      async function getTags() {
        ...
        const tagTarget = document.querySelector(
          "#content > table > tbody > tr > td:nth-child(1)"
        );
        const tagLocalForage = localforage.createInstance({
          name: "MalJS",
          storeName: "tags",
        });
        ...
        if (!tagCache || tagCache.time + tagcacheTTL < +new Date()) {
          ...
          if (
            tagData?.data.Media &&
            tagData.data.Media.tags &&
            tagData.data.Media.tags.length > 0
          ) {
        ...
        if (tagCache) {
          if (
            tagTarget.lastChild.lastElementChild &&
            tagTarget.lastChild.lastElementChild.className ===
              "clearfix mauto mt16"
          ) {
          ...
          if (
            tagTarget.lastChild.lastElementChild &&
            tagTarget.lastChild.lastElementChild.className !== "pb16"
          ) {
          ...
          tagDiv.innerHTML += tagCache.tags
            .map(
              (node) => `
            <div class="${
              node.isMediaSpoiler === true ? "aniTag spoiler" : "aniTag"
            }"><a title="${
                node.description ? node.description : ""
              }"><div class="aniTag-name">${node.name.replace(
                /'/g,
                " "
              )}</div></a>
          ...
          if ($(".aniTagDiv .spoiler").length) {
            let showSpoilers = create(
              "div",
              { class: "showSpoilers" },
              "Show " +
                $(".aniTagDiv .spoiler").length.toString() +
                " spoiler tags"
            );
    ...
    if (svar.animeRelation) {
      ...
      async function getRelations() {
        ...
        const relationTargetExpand = create("a", {
          class: "relations-accordion-button",
        });
        const extraRelationsDiv = create("div", {
          class: "relationsExpanded",
          style: { display: "none" },
        });
        const relationTarget = document.querySelector(".related-entries");
        const relationLocalForage = localforage.createInstance({
          name: "MalJS",
          storeName: "relations",
        });
        const relationcacheTTL = 262974383;
        let relationCache = await relationLocalForage.getItem(
          entryId + "-" + entryType
        );
        const priorityOrder = {
          ADAPTATION: 0,
        ...
        if (
          !relationCache ||
          relationCache.time + relationcacheTTL < +new Date()
        ) {
          relationData = await aniAPIRequest();
          relationData?.data.Media
            ? (relationData = relationData.data.Media.relations.edges.filter(
                (node) => node.node.idMal !== null
              ))
            : null;
          if (relationData && relationData.length > 0) {
            ...
            relationCache = await relationLocalForage.getItem(
              entryId + "-" + entryType
            );
          }
        }
        if (relationCache && relationTarget) {
          ...
          document.querySelector(
            "#content > table > tbody > tr > td:nth-child(2) > div.rightside.js-scrollfix-bottom-rel > table"
          ...
          relationTarget.innerHTML = relationCache.relations
            .map((node) => {
              ...
              const format = node.node.format
                ? node.node.format === "NOVEL"
                  ? (node.node.format = "LIGHT NOVEL")
                  : node.node.format.replace("_", " ")
                : node.node.type;
              const coverImage =
                node.node.coverImage && node.node.coverImage.large
                  ? node.node.coverImage.large
                  : node.node.coverImage.medium
                  ? node.node.coverImage.medium
                  : "";
              const borderColor = isManga ? "#92d493" : "#afc7ee";
              const relationType = node.relationType.split("_").join(" ");
              const title =
                node.node.title && node.node.title.romaji
          ...
          function relationDetailsShow() {
            $(".relationEntry").on("mouseenter", async function () {
              ...
              const isOverflowing =
                divRect.left < 0 || divRect.right > viewportWidth;
              if (isOverflowing) {
          ...
          if (relationTarget.clientHeight > 140) {
            ...
            const extraRelations =
              relationTarget.querySelectorAll(".relationEntry");
            relationTargetExpand.innerHTML =
              '<i class="fas fa-chevron-down mr4"></i>\nShow More\n';
            for (let i = 0; i < extraRelations.length; i++) {
              if (relationTarget.clientHeight > 144) {
                extraRelationsDiv.appendChild(
                  relationTarget.querySelector(".relationEntry:last-child")
                );
              }
            }
            ...
            relationTarget.insertAdjacentElement(
              "afterend",
              relationTargetExpand
            );
            relationTarget.querySelector("div:nth-child(1)").style.marginLeft =
              "8px";
            extraRelationsDiv.setAttribute("style", "display:none!important");
            relationTarget.setAttribute(
              "style",
              "margin-bottom:5px;padding:12px 4px!important"
            );
            relationTargetExpand.addEventListener("click", function () {
              if (
                document.querySelector(".relationsExpanded").style.display ===
                "none"
              ) {
                document
                  .querySelector(".relationsExpanded")
                  .setAttribute("style", "display:flex!important");
                relationTargetExpand.innerHTML =
                  '<i class="fas fa-chevron-up mr4"></i>\nShow Less\n';
              } else {
                document
                  .querySelector(".relationsExpanded")
          ...
          let filterTarget = document.querySelector(
            ".RelatedEntriesDiv .floatRightHeader"
          );
          if (filterTarget && svar.relationFilter && svar.animeRelation) {
            ...
            extraRelationsDiv.setAttribute(
              "style",
              "display: flex!important;height: 0px;overflow: hidden;"
            );
            ...
            function filterRelations(title) {
              if (!relationFilter.children[1].value.length) {
                ...
                if (!filtered) {
                  relationTarget.innerHTML =
                    relationDefault + extraRelationsDiv.innerHTML;
                  filtered = 1;
                }
              }
              ...
              for (let i = 0; i < entries.length; i++) {
                ...
                const relationTitle =
                  entry.querySelector(".relationTitle").textContent;
              ...
              if (!relationFilter.children[1].value.length) {
                ...
                relationTargetExpand.setAttribute(
                  "style",
                  "display:block!important"
                );
                relationTargetExpand.innerHTML =
            ...
            function updateFilterOptions() {
              ...
              const titles = Array.from(
                document.querySelectorAll(".relationTitle")
              ...
              if (
                document.querySelectorAll("#relationFilter option").length <= 2
              ) {
                document.querySelector(".relations-filter").remove();
              } else {
                document
                  .querySelector(".RelatedEntriesDiv")
                  .setAttribute(
                    "style",
                    "align-content: center;margin-bottom: 10px;"
                  );
                document
                  .querySelector(".RelatedEntriesDiv #related_entries")
    ...
    if (!/(.*anime|manga)\/.*\/.*\/\w.*/gm.test(current)) {
      let main = document.querySelector(
        "#content > table > tbody > tr > td:nth-child(2)[valign='top'] tr > td[valign='top']"
      );
      for (let x = 0; x < 1; x++) {
      ...
      let textfix = text.innerHTML
        .replace(/<br>.*\s/gm, "")
        .replace(/\n\s{3,10}/g, "");
      if (textfix.includes("No background")) {
        textfix = textfix.replace(
          /(information here.+)/gm,
          'information <a href="/dbchanges.php?aid=' +
            entryId +
            '&amp;t=background">here</a>.'
        );
      }
  ...
  if (/\/(anime|manga)\/producer\/\d.?([\w-]+)?\/?/.test(current)) {
    ...
    if (
      $(studioDivShadow).length &&
      $(studioDivShadow).children().css("flex") !== "1 1 0%"
    ) {
      $(studioDivShadow)
        .children()
        .attr("style", "background:0!important")
  ...
  if (/\/(people)\/.?([\w-]+)?\/?/.test(current)) {
    ...
    let peopleDivShadow = document.querySelector(
      "#content > table > tbody > tr > td.borderClass  .spaceit_pad"
    );
    if (peopleDivShadow) {
      $(peopleDivShadow).attr("style", "background:0!important");
      $(peopleDivShadow)
        .nextUntil("div:not(.spaceit_pad)")
        .attr("style", "background:0!important")
        .addBack()
        .wrapAll("<div class='spaceit-shadow-end-div'></div>");
      $('div:contains("Website:"):last').html() ===
      'Website: <a href="http://"></a>'
        ? $('div:contains("Website:"):last').remove()
        : null;
      $('div:contains("Family name:"):last').html() === "Family name: "
        ? $('div:contains("Family name:"):last').remove()
        : null;
      $('span:contains("More:"):last').css({
        display: "block",
        padding: "2px",
        marginTop: "5px",
      });
  ...
  if (
    (/\/(people)\/.?([\w-]+)?\/?/.test(current) && svar.peopleHeader) ||
    (/\/(character)\/.?([\w-]+)?\/?/.test(current) && svar.characterHeader)
  ) {
    ...
    table.prepend(name);
    if (
      /\/(character)\/.?([\w-]+)?\/?/.test(current) &&
      !/\/(clubs)/.test(current) &&
      !/\/(pics)/.test(current) &&
      svar.characterHeader
    ) {
      if (!svar.characterNameAlt) {
        name.setAttribute("style", "line-height:25px");
      }
      let extra = document.querySelector(
        "#content > table > tbody > tr > td.characterDiv > h2 > span > small"
      );
      if (extra) {
      ...
      if (svar.characterNameAlt) {
        if (extra) {
          ...
          document
            .querySelector(".h1.edit-info > div.h1-title > h1")
            .append(extra);
          extra.style.lineHeight = "20px";
          if (
            name.children[0].children[0].children[0].innerText.match(/".*"/gm)
          ) {
            extra.innerHTML =
              extra.innerHTML +
              "</br>" +
              name.children[0].children[0].children[0].innerText.match(
                /".*"/gm
              );
            name.children[0].children[0].children[0].innerText =
              name.children[0].children[0].children[0].innerText.replace(
                /".*"/gm,
                ""
              );
          } else {
      ...
      document
        .querySelector("#content > table > tbody > tr > td.characterDiv > h2")
        .remove();
    }
  }
  ...
  function headerPosChange(v) {
    if (
      ...
      set(
        1,
        "#content > table > tbody > tr > td:nth-child(2) > .js-scrollfix-bottom-rel",
        { pp: { 0: ".h1.edit-info" } }
      );
      const titleOldDiv = document.querySelector(
  ...
  if (/\/(clubs.php).?([\w-]+)?\/?/.test(current)) {
    ...
    $('div.normal_header:contains("Club Pictures")')
      .next()
      .children()
      .children()
      .children()
      .addClass("clubDivs");
    $("#content > table > tbody > tr > td[valign=top]:last-child").addClass(
      "clubDivs"
    );
    set(2, ".clubDivs", {
      sal: { 0: "border-radius:var(--br);overflow:hidden" },
    });
  }
  ...
  if (
    svar.blogRedesign &&
    (/\/(blog)\//.test(current) || /\?eid=/.test(location.search))
  ) {
    ...
    $(".lightLink:not(.lightLink.to-left)").each(function () {
      ...
      $(this).wrap(function () {
        let hrefAttribute =
          !/\?eid=/.test(location.search) && headerHref
            ? `href="${headerHref}"`
            : "";
    ...
    $('.normal_header:not(:contains("Categories"))').each(function () {
      $(this)
        .nextUntil(".borderClass")
        .last("div")
        .addClass("maljsBlogDivContent");
      $(this)
        .nextUntil(".borderClass")
        .wrapAll('<div class="maljsBlogDiv"></div>');
    });

    $(".maljsBlogDivHeader:not(.maljsBlogDiv .maljsBlogDivHeader)").each(
      function () {
        $(this)
          .nextUntil(".borderClass")
          .last("div")
    ...
    //wrap relations div
    $('.maljsBlogDiv div:contains("Relations:")').wrap(
      '<div class="maljsBlogDivRelations"></div>'
    );
  ...
  if (
    (/\/(blog.php)/.test(current) || /\/(blog)\//.test(current)) &&
    !/\?eid=/.test(location.search)
  ) {
  ...
  if (
    /myanimelist.net\/(anime|manga|character|people)\/?([\w-]+)?\/?/.test(
      location.href
    ) &&
    !document.querySelector("#content > .error404")
  ) {
    ...
    if (
      ...
      /\/(anime|manga)\/producer|season|genre|magazine\/.?([\w-]+)?\/?/.test(
        current
      ) ||
  ...
  if (svar.animeSongs) {
    ...
    function anisong() {
      ...
      if (currentpath && currentpath[1] === "anime") {
        ...
        anisongs_temp.target = document.querySelector(
          ".rightside.js-scrollfix-bottom-rel div.di-t:not(.w100)"
        );
      ...
      const API = {
        ...
        async getSongs(mal_id) {
          const res = await fetch(
            `https://api.jikan.moe/v4/anime/${anisongs_temp.id}/themes`
          );
          return res.json();
        },
        ...
        async getVideos(anilist_id) {
          ...
          const res = await fetch(
            `https://api.animethemes.moe/anime?filter[has]=resources&filter[site]=MyAnimeList&filter[external_id]=${anisongs_temp.id}&include=${include}`
          );
          return res.json();
        },
      };
      ...
      class Videos {
        ...
        async get() {
          ...
          let d = anime
            ? anime[0].animethemes.sort((a, b) => a.sequence - b.sequence)
            : null;
          let t = [];
        ...
        static merge(entries, videos) {
          ...
          const findUrl = (n) => {
            ...
            if (videos[n]) {
              if (
                videos[n].animethemeentries[0] &&
                videos[n].animethemeentries[0].videos[0]
              ) {
          ...
          if (videos) {
            return entries.map((e, i) => {
              ...
              for (let x = 0; x < videos.length; x++) {
                let vid = videos[x];
                let link =
                  vid.animethemeentries[0].videos[0] &&
                  vid.animethemeentries[0].videos[0].link
                    ? vid.animethemeentries[0].videos[0].link
                    : null;
                let m = 0;
                let title = cleanTitle(e)
                  .replace(/(\w\d+\: |)/gm, "")
                  .replace(/\((?!.*(Ver\.|ver\.))(.*?)\)+?/g, "")
                  .replace(/(.*)( by )(.*)/g, "$1")
                  .replace(/(.*)( feat. | ft. )(.*)/g, "$1")
                ...
                let ep2 =
                  epdata &&
                  (epdata.constructor !== Array || epdata.length === 1)
                    ? epdata.constructor !== Array
                      ? epdata.replace(/\s/g, "")
                      : epdata
                    : null;
                let eps = [];
                ...
                if (m === 0 && vid.sequence) {
                  if (
                    i + 1 === vid.sequence &&
                    stringSimilarity(title, vid.song.title) > 0.8
                  ) {
                    u = link;
                    m = 1;
                  }
                ...
                if (
                  ...
                  if (
                    ...
                    (stringSimilarity(artist, artists) > 0.82 ||
                      ...
                      (matches.length > 0 &&
                        stringSimilarity(artists, matches) > 0.82))
                  ) {
                    artistmatch = 1;
                    if (
                      stringSimilarity(title, vid.song.title) > 0.8 ||
                      (i === vid.sequence &&
                        stringSimilarity(title, title2) > 0.8) ||
                      (!vid.sequence && stringSimilarity(title, title2) > 0.8)
                    ) {
                      u = link;
                      m = 1;
                    }
                ...
                if (
                  ...
                  ((vid.sequence &&
                    ...
                    vid.slug &&
                    videos.length < 10) ||
                    (!vid.sequence && vid.slug && videos.length < 10))
                ) {
                  if (
                    anisongdata &&
                    anisongdata.openings.length > 0 &&
                    vid.type === "OP"
                  ) {
                    let n = vid.slug.replace(/(OP)(.*\d)(.*)/g, "$2");
                    if (
                      n === (i + 1).toString() &&
                      (!vid.sequence || (artistmatch && i + 1 === vid.sequence))
                    ) {
                      u = link;
                      m = 1;
                    }
      ...
      function insert(songs, parent) {
        if (!songs || !songs.length) {
          ...
          songs.forEach((song, i) => {
            ...
            if (song.url) {
              let play = create("div", {
                class: "oped-preview-button oped-preview-button-gray",
              });
              node.prepend(play);
      ...
      function cleaner(target) {
        ...
        $(
          ".rightside.js-scrollfix-bottom-rel div.di-t > .di-tc.va-t:has(h2)"
        ).remove();
      ...
      function placeData(data) {
        ...
        async function addAccordion(div) {
          ...
          if (themeSongs.length > 4) {
            const accordionButton = create("a", {
              class: "anisong-accordion-button",
              style: { display: "none" },
            });
            const extraSongs = create("div", {
              class: "anisong-extra-songs",
              style: { display: "none" },
            });
          ...
          for (let x = 0; x < themeSongs.length; x++) {
            ...
            favorite.onclick = async () => {
              ...
              const animeTitle = $(".title-name").text()
                ? $(".title-name").text()
                : document.title.replace(" - MyAnimeList.net", "");
              ...
              async function imgLoad() {
                img = document.querySelector("div:nth-child(1) > a > img");
                set(0, img, {
                  sa: { 0: "position: fixed;opacity:0!important;" },
                });
                if (img && img.src) {
                  set(0, img, {
                    sa: { 0: "position: relative;opacity:1!important;" },
            ...
            if (
              themeSongs[x].className ===
                "theme-songs js-theme-songs has-video" &&
              $(".header-profile-link").text() !== ""
            ) {
        ...
        if (aniSongsMainDiv) {
          ...
          let AniSongsFooter = create(
            "div",
            {
              class: "anisongs-footer",
              style: { textAlign: "right", marginRight: "5px" },
            },
            "Themes provided from " +
              '<a href="https://animethemes.moe/">AnimeThemes.moe</a><br>' +
              "Last Update: " +
              lastCheck +
              " "
          );
      ...
      async function launch(currentid) {
        ...
        if (cache.time + options.cacheTTL < +new Date()) {
          ...
          if (
            mal_id &&
            ["Finished Airing", "Currently Airing"].includes(status)
          ) {
