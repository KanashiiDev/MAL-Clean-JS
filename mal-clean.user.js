...
function create(e, t, n) {
	...
	if (t)
		for (r in t)
			if ("style" === r)
				for (i in t.style) f.style[i] = t.style[i];
...
button11.onclick = () => {
	if (bginput.value.slice(-1) === "]") {
		...
		bginput.value =
			"[url=https://custombg/" +
			LZString.compressToBase64(JSON.stringify(bginput.value)) +
			"]‎ [/url]";
		bginput.select();
		bginfo.innerHTML =
...
button12.onclick = () => {
	if (pfinput.value.slice(-1) === "]") {
		...
		pfinput.value =
			"[url=https://custompf/" +
			LZString.compressToBase64(JSON.stringify(pfinput.value)) +
			"]‎ [/url]";
		pfinput.select();
		pfinfo.innerHTML =
...
button8.onclick = () => {
	if (cssinput.value.slice(-1) === "]") {
		...
		cssinput.value =
			"[url=https://customcss/" +
			LZString.compressToBase64(JSON.stringify(cssinput.value)) +
			"]‎ [/url]";
		cssinput.select();
		cssinfo.innerHTML =
			"Css Converted. Please copy and paste to your " +
			"<a class='embedLink' href=\"" +
			"https://myanimelist.net/editprofile.php" +
			'">' +
			"About Me" +
...
function add() {
	var header = document.querySelector(
		"#header-menu > div.header-menu-unit.header-profile.js-header-menu-unit.link-bg.pl8.pr8.on > div > ul > li:nth-child(9)"
	);
	if (!header) {
		setTimeout(add, 100);
		return;
	...
	var gear = document.querySelector(
		"#header-menu > div.header-menu-unit.header-profile.js-header-menu-unit.link-bg.pl8.pr8.on > div > ul > li:nth-child(9) > a > i"
	);
...
//Anilist Style Loading Button Animation Fix
let v = false;
let lv = 0;
...
(function() {
	...
	if (/\/(profile)\/.?([\w-]+)?\/?/.test(current)) {
		...
		async function findbg() {
			...
			if (document.querySelector("#fancybox-loading")) {
				document
					.querySelector("#fancybox-loading")
			...
			if (about) {
				...
				if (m) {
					...
					banner.setAttribute(
						...
						"background:url(" +
						custombg +
						");background-position: 50% 35%; background-repeat: no-repeat;background-size: cover;height: 330px;position: relative;"
					);
					svar.alstyle = true;
				}
				if (m2) {
        ...
        #horiznav_nav .navactive {color: var(--color-text)!important;background: var(--color-foreground2)!important;padding: 5px!important;}`;
						...
						.then((data) => {
							...
							if (entry[0]) {
								...
								document
									...
									.insertAdjacentElement("beforeend", head);
								myLoop();
							}
							...
							async function getimgf() {
								...
								if (
									i < last &&
									i > 0 &&
									entry[i].entry.mal_id !== entry[i - 1].entry.mal_id
								) {
									getimg();
								} else {
									if (i < last && i > 0) {
										...
										document
											.querySelector("#statistics")
											.insertAdjacentElement("beforeend", dat);
										i++;
										if (i < last) {
											myLoop();
										}
				...
				if (
					set(0, modernabout, {
						sa: {
							0: "margin-bottom: 20px;width: auto;background: var(--color-foreground);padding: 10px;border-radius: var(--br);max-height:2000px"
						}
					})
				) {
					document
				...
				if (
					...
					document
						.querySelector("#content > div > div.container-right")
						.prepend(document.querySelector("#lastcomment"));
				}
				...
				document
					...
					.setAttribute(
						"style",
						"width: 1375px;min-width:500px; margin: auto;top: -40px;transition:.6s"
					);
				if (document.querySelector("#fancybox-loading")) {
					document
						.querySelector("#fancybox-loading")
				...
				if (s) {
					...
					s.children[1].append(
						document.querySelector("#statistics .stats.manga")
					);
					s.children[2].prepend(
						document.querySelector("#statistics .updates.anime")
					);
					...
					document
						.querySelector(
							"#content > div > div.container-left > div > ul:nth-child(4)"
						)
						.prepend(favs, favs2, favs3, favs4, favs5);
					getfavs();
				...
				nav.setAttribute(
					"style",
					"z-index: 2;position: relative;background: #000;padding: 10px;text-align: center;background-color: var(--color-foreground) !important;"
				);
		...
		if (svar.profileHeader) {
			...
			if (!table) {
				table = document.querySelector(
					"#content > div > div.container-right > div > div:nth-child(1)"
				);
			}
			if (table) {
				table.prepend(title);
	...
	if (/\/(character)\/.?([\w-]+)?\/?/.test(current)) {
		...
		if (
			document.querySelector(
				"#content > table > tbody > tr > td.characterDiv > br:nth-child(5)"
			)
		) {
			...
			doc = document.querySelector(
				"#content > table > tbody > tr > td.characterDiv > br:nth-child(6)"
			);
		}
		doc.before(text);
		$.trim(
	...
	if (
		...
		!/\/(anime|manga)\/producer\/.?([\w-]+)?\/?/.test(current)
	) {
		let text = create("div", {
			class: "description",
			itemprop: "description",
		...
		if ($('h2:contains("Alternative Titles"):last')) {
			$('h2:contains("Alternative Titles"):last').addClass(
			...
			document
				...
				.nextElementSibling.setAttribute(
					"style",
					"border-radius:var(--br);margin-bottom:2px"
				);
		...
		document
			.querySelector(".InformationDiv")
			.nextElementSibling.setAttribute(
				"style",
				"border-top-left-radius:var(--br);border-top-right-radius:var(--br);"
			);
		...
		//Right Side
		$('h2:contains("Synopsis"):last').parent().addClass("SynopsisDiv");
		$('h2:contains("Episode Videos"):last')
			.parent()
			.addClass("EpisodeVideosDiv");
		...
		$('h2:contains("Recommendations"):last')
			.parent()
			.addClass("RecommendationsDiv");
		$(".RecommendationsDiv")
			.closest("div")
			.append($(".RecommendationsDiv").next());
		$('h2:contains("Interest Stacks"):last')
			.parent()
			.addClass("InterestStacksDiv");
		$(".InterestStacksDiv")
			.closest("div")
			.append($(".InterestStacksDiv").next());
		$('h2:contains("Recent News"):last').addClass("RecentNewsDiv");
		$('h2:contains("Recent Featured Articles"):last')
			.parent()
			.addClass("RecentFeaturedArticlesDiv");
		$('h2:contains("Recent Forum Discussion"):last').addClass(
			"RecentForumDiscussionDiv"
		);
		$('h2:contains("MALxJapan -More than just anime-"):last')
			.parent()
			.addClass("MalxJDiv");
		...
		if (!/(.*anime|manga)\/.*\/.*\/\w.*/gm.test(current)) {
			...
			for (let x = 0; x < 100; x++) {
				if (
					$(
						'.backgroundDiv:contains("No background information has been added to this title.")'
					).css("display", "none")
				);
			}
		}
	...
	function anisong() {
		...
		if (currentpath && currentpath[1] === "anime") {
			...
			anisongs_temp.target = document.querySelector(
				".rightside.js-scrollfix-bottom-rel > table > tbody > tr:nth-child(3) div.di-t"
			);
		...
		const API = {
			async getSongs(mal_id) {
				const res = await fetch(
					`https://api.jikan.moe/v4/anime/${currentid}/themes`
				);
				return res.json();
			},
			async getVideos(anilist_id) {
				const res = await fetch(
					`https://api.animethemes.moe/anime?filter[has]=resources&filter[site]=MyAnimeList&filter[external_id]=${currentid}&include=animethemes.animethemeentries.videos`
				);
				return res.json();
			}
		};
		...
		class Videos {
			...
			async get() {
				...
				return Videos.groupTypes(anime[0].animethemes.sort((a, b) => a.sequence - b.sequence));
			}
		...
		function cleaner(target) {
			...
			$(
				".rightside.js-scrollfix-bottom-rel > table > tbody > tr:nth-child(3) > td > div.di-t > .di-tc.va-t"
			).remove();
			set(
				1,
				".rightside.js-scrollfix-bottom-rel > table > tbody > tr:nth-child(3) > td > div.di-t", {
					sa: {
			...
			$(
				".rightside.js-scrollfix-bottom-rel .di-b.ar"
			).remove();
		}
		...
		async function launch(currentid) {
			...
			if (cache.time + options.cacheTTL < +new Date()) {
				...
				if (mal_id) {
					const {
						data
					} = await API.getSongs(mal_id);
					let {
						openings: opening_themes,
						endings: ending_themes
					} = data;
