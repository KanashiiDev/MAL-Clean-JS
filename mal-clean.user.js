...
button1.onclick = () => {
  animebg = checkdata(animebg, "animebg");
  getSettings();
  reloadset();
};
var button2 = create(
...
button2.onclick = () => {
  animeHeader = checkdata(animeHeader, "animeHeader");
  getSettings();
  reloadset();
};
var button3 = create(
...
button3.onclick = () => {
  charbg = checkdata(charbg, "charbg");
  getSettings();
  reloadset();
};
var button4 = create(
...
button4.onclick = () => {
  characterHeader = checkdata(characterHeader, "characterHeader");
  getSettings();
  reloadset();
};
var button5 = create(
...
button5.onclick = () => {
  characterNameAlt = checkdata(characterNameAlt, "characterNameAlt");
  getSettings();
  reloadset();
};
var button6 = create(
...
button6.onclick = () => {
  peopleHeader = checkdata(peopleHeader, "peopleHeader");
  getSettings();
  reloadset();
};
var button7 = create(
...
button7.onclick = () => {
  customcss = checkdata(customcss, "customcss");
  getSettings();
  reloadset();
};
var button9 = create(
...
button9.onclick = () => {
  profileHeader = checkdata(profileHeader, "profileHeader");
  getSettings();
  reloadset();
};
var button10 = create(
...
button10.onclick = () => {
  alstyle = checkdata(alstyle, "alstyle");
  getSettings();
  reloadset();
};
//alstyle - BG Elements
...
//Check LocalStorage for saved variables
...
(function () {
  ...
  if (
    (/\/(people)\/.?([\w-]+)?\/?/.test(current) && peopleHeader) ||
    (/\/(character)\/.?([\w-]+)?\/?/.test(current) && characterHeader)
  ) {
  ...
  if (
    ...
    if (/\/(character)\/?([\w-]+)?\/?/.test(current) && !charbg) {
      return;
    }
    if (/\/(anime|manga)\/?([\w-]+)?\/?/.test(current) && !animebg) {
      return;
    }
